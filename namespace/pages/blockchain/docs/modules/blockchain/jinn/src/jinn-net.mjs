...
//Engine context
    ...
    function InitClass(Engine) {
        ...
        Engine.ReceiveFromNetwork = function (Child, Data) {
            debug(-4, '🌶 [(Engine)ReceiveFromNetwork]', Data)
        ...
        Engine.PrepareOnSend = function (Method, Child, DataObj, bCall, F, RetContext) {
            debug(-4, '🌶 [(Engine)PrepareOnSend]', DataObj)
            if (typeof process !== "object")
        ...
        Engine.ReceiveFromNetworkNext = function (Child, Data) {
            debug(-4, '🌶 [(Engine)ReceiveFromNetworkNext]',Data)
        ...
        Engine.PrepareOnSendNext = function (Method, Child, DataObj, bCall, F, RetContext) {
            debug(-4, '-------------🌶[(PrepareOnSendNext*) ip]', Child.ip)
            ...
            if (global.TEST_NET_DEBUG) {
                global.WriteUint32AtPos(TEMP_PACKET_ARR, Child.SendPacketCount, 0);
                global.WriteUint32AtPos(TEMP_PACKET_ARR, 8 + DataBuf.length, 4);
            } else {
                global.WriteUint32AtPos(TEMP_PACKET_ARR, 4 + DataBuf.length, 0);
            }
        ...
        Engine.TweakOneMethod = function (Child) {
            ...
            if (global.TEST_NET_DEBUG) {
                ...
                var PacketNum = global.ReadUint32FromArr(Arr, 0);
                Length = global.ReadUint32FromArr(Arr, 4);
                ...
                Length = global.ReadUint32FromArr(Arr, 0);
                Pos = 4;
            }

            if (Length > global.JINN_CONST.MAX_PACKET_SIZE) {
        ...
        Engine.RunMethod = function (Method, F, Child, Data, bCall, DataLength) {
            if (typeof process === "object") {
                ...
                var DeltaReads = global.JINN_STAT.ReadRowsDB - WasReads;
                ...
                if (bCall)
                    global.JINN_STAT.TimeCall += deltaTime;
                else
                    global.JINN_STAT.TimeRet += deltaTime;
        ...
        Engine.GetRAWFromObject = function (Child, Method, bCall, RetContext, DataObj) {
            ...
            return global.SerializeLib.GetBufferFromObject(Obj, NetFormat, NetFormatWrk);
        };

        Engine.GetObjectFromRAW = function (BufData) {
            debug(-4, '🌶 [(Engine)GetObjectFromRAW]',BufData)
            if (NET_STRING_MODE) {
                var Str = Engine.GetJSONFromRAW(BufData);
                return JSON.parse(Str);
            }

            var Obj = global.SerializeLib.GetObjectFromBuffer(BufData, NetFormat, NetFormatWrk);
        ...
        Engine.LogBufTransfer = function (Child, Data, StrDirect) {
            ...
            if (0 && global.DEBUG_ID && Data) {
                Engine.ToDebug(StrDirect + Child.ID + " " + global.GetHexFromArr(Data) + " - " + Data.length + "/" + Engine.Traffic);
            }
        };
        ...
        Engine.AddTraffic = function (Count) {
            ...
            global.JINN_STAT.AllTraffic += Count;
            ...
            for (var i = 0; i < LevelArr.length; i++) {
                ...
                if (Child) {
                    for (var Key in Child.ContextCallMap) {
                        ...
                        if (Delta > global.JINN_CONST.METHOD_ALIVE_TIME) {
                            delete Child.ContextCallMap[Key];
