...
import cdbResult from "../jinn/tera/db/tera-db-result.mjs";
import commonEvent from './common-event.mjs'
...
debug('[(self)a]', global.PROCESS_NAME)
    commonEvent(global)
    var BufHashTree = new global.RBTree(global.CompareArr);
    BufHashTree.LastAddNum = 0;

    const CDBResult = cdbResult(global)
    ...
    function GET_DAPP_TRANSACTIONS(Block)
    {
        ...
        var Arr1 = global.SERVER.DoCreateCrossArr(Block);
        if(Arr1)
        {
        ...
        var Tx2 = global.ACCOUNTS.GetAccountHashTx(Block.BlockNum);
        if(Tx2)
        {
            Arr.push(Tx2);
        }
    ...
    function BLOCK_PROCESS_TX(Block)
    {
        ...
        {
            global.DApps[key].OnProcessBlockStart(Block);
        }
        ...
        for(var key in global.DApps)
        {
            global.DApps[key].OnProcessBlockFinish(Block);
        }
    ...
    function BlockTruncate(BlockNum)
    {
        for(var key in global.DApps)
        {
            global.DApps[key].OnDeleteBlock(BlockNum);
        }
