...
let debug = (maxCount, id, props, data, ...args) => {
    ...
    let from = path.search('/system');
    let to = path.length;
    let url = path.substring(from,to);
    logs.assert(-1,url, id, props, data, args)
}
export default (global) => {
debug('[(self)a]', global.PROCESS_NAME)
    let DAPP = global.DApp
    ...
    class SysCoreApp extends DAPP {
        constructor() {
            ...
            var FormatRow=global.CopyObjKeys({BlockNum:"uint",TrNum:"uint16"},global.FORMAT_SYS);
            delete FormatRow.OperationID;
            delete FormatRow.Sign;

            this.DB = new global.CDBRow("syscore", FormatRow, bReadOnly, "Num", 10, 0, 0)
            global.REGISTER_TR_DB(this.DB, 60);
        }
        ...
        TRNewVerson(Block, Body, BlockNum, TrNum, ContextFrom)
        {
            ...
            var TR = global.SerializeLib.GetObjectFromBuffer(Body, global.FORMAT_SYS, WorkStructSys);
    ...
    global.REGISTER_SYS_DAPP(App, global.TYPE_TRANSACTION_SYS);
    return  SysCoreApp;
}