...
// import teraDbTree from '../jinn/tera/db/tera-db-tree.mjs'
import shardCrossLib from "./shard-cross-lib.mjs"
...
debug('[(self)a]', global.PROCESS_NAME)
    const CDBCrossTx = global.CDBCrossTx
    const CDBTree = global.CDBTree
    const ShardCrossLib = shardCrossLib(global)
    class ShardCrossDB extends ShardCrossLib
    {
        constructor(bReadOnly)
        {
            ...
            global.REGISTER_TR_DB(this.CrossRowHashBlock.DB, 56)
            global.REGISTER_TR_DB(this.CrossDataHashRun.DB, 58)
            this.ShardProcessingTree = new global.RBTree(function (a,b)
            {
