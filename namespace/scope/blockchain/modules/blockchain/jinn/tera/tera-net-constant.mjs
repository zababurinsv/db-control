...
debug('[(self)a]', global.PROCESS_NAME)
    ...
    {
        global.JINN_NET_CONSTANT = CopyNetConstant({}, global.JINN_CONST);
        global.JINN_NET_CONSTANT.NetConstVer = 0;
        global.JINN_NET_CONSTANT.NetConstStartNum = 0;
        ...
        Engine.ProcessNetConstant = function (Child,NetConstVer)
        {
            ...
            if(NetConstVer > global.JINN_NET_CONSTANT.NetConstVer)
            {
        ...
        {
            if(Version > global.GetCurrentBlockNumByTime() + 3000)
                return;
            ...
            Engine.Send("NETCONSTANT", Child, {NetConstVer:Version}, function (Child,Data)
            {
                ...
                if(Data.NetConstVer > global.JINN_NET_CONSTANT.NetConstVer)
                {
        ...
        {
            var RetData = CopyNetConstant({}, global.JINN_NET_CONSTANT);
            return RetData;
        };
        ...
        Engine.CheckNetConstant = function (Data)
        {
            ...
            if(!global.CheckDevelopSign(SignArr, Data.NET_SIGN))
            {
                return 0;
            }
            ...
            global.ToLog("Got NEW NetConstant (wait " + Delta + " blocks) Ver: " + Data.NetConstVer, 2);

            CopyNetConstant(global.JINN_NET_CONSTANT, Data);
        ...
        {
            var Buf = global.SerializeLib.GetBufferFromObject(Data, FormatForSign, {});
            return global.sha3(Buf, 12);
        };

        Engine.DoNetConst = function () {
            global.ToLog("DoNetConstant: " + global.JINN_NET_CONSTANT.NetConstVer, 2);

            var WasConst = CopyNetConstant({}, global.JINN_CONST);

            CopyNetConstant(global.JINN_CONST, global.JINN_NET_CONSTANT);

            if(!global.JINN_CONST.MAX_TX_SIZE)
                global.JINN_CONST.MAX_TX_SIZE = 64000;
            var CountCreate = global.JINN_NET_CONSTANT.TEST_COUNT_BLOCK + global.JINN_NET_CONSTANT.NetConstStartNum - Engine.CurrentBlockNum;
            if(global.JINN_NET_CONSTANT.TEST_COUNT_BLOCK && CountCreate > 0)
            {
                ToLog("*************CountBlockCreate: " + CountCreate + " witch TX=" + global.JINN_CONST.TEST_COUNT_TX, 2);
                var Num = 10 + global.random(global.ACCOUNTS.GetMaxAccount() - 10);
                global.SendTestCoin(Num, Num, 1, global.JINN_CONST.TEST_COUNT_TX, CountCreate, 1);
            }
            if(global.JINN_NET_CONSTANT.RUN_RESET && (global.JINN_NET_CONSTANT.NetConstStartNum + 1) >= Engine.CurrentBlockNum)
            {

                if(global.JINN_NET_CONSTANT.RUN_RESET === 5)
                {
                    ToLog("****REWRITE_DAPP_TRANSACTIONS*****", 2);
                    if(!global.DEV_MODE)
                        REWRITE_DAPP_TRANSACTIONS(5000);
                }
                if(global.JINN_NET_CONSTANT.RUN_RESET === 10)
                {
                    ToLog("****ClearCommonStat*****", 2);
                    global.ClearCommonStat();
                }
                if(global.JINN_NET_CONSTANT.RUN_RESET === 20)
                {
                    ToLog("****RewriteAllTransactions*****", 2);
                    if(!global.DEV_MODE)
                        SERVER.RewriteAllTransactions();
                }
                if(global.JINN_NET_CONSTANT.RUN_RESET === 30)
                {
                    ToLog("****Exit*****", 2);
                    if(!global.DEV_MODE)
            ...
            if(WasConst.TX_PRIORITY_MODE !== global.JINN_CONST.TX_PRIORITY_MODE || WasConst.TX_PRIORITY_LENGTH !== global.JINN_CONST.TX_PRIORITY_LENGTH)
            {
