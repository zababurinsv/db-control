...
debug('[(self)a]', global.PROCESS_NAME)
    ...
    let Map404 = Object.assign(Object.create(null), {});
    ...
    function GET_ACCOUNT(Obj)
    {
        ...
        let GET_PROP = {get Num()
            ...
            }, get PubKeyStr()
            {
                ...
                {
                    return global.GetHexFromArr(Data.PubKey);
                }
            }, get Name()
            {
                return Data.Name;
    ...
    {
        let arr = Object.getOwnPropertyNames(Value);
        for(let name of arr)
        {
    ...
    ListF.$SetValue = function (ID,CoinSum)
    {
        ...
        let ToData = global.ACCOUNTS.ReadStateTR(ID);
        if(!ToData)
        {
        ...
        global.ACCOUNTS.WriteStateTR(ToData, RunContext.BlockNum, RunContext.TrNum);
    ...
    ListF.$Send = function (ToID,CoinSum,Description,Currency,TokenID)
    {
        ...
        let ToData = global.ACCOUNTS.ReadStateTR(ToID);
        if(!ToData)
        {
        ...
        return global.ACCOUNTS.SendMoneyTR(RunContext.Block, RunContext.Account.Num, ToID, CoinSum, RunContext.BlockNum, RunContext.TrNum, Description, Description, 1, 1,0,Currency,TokenID);
    };
    ...
    function DoTransfer(FromID,ToID,CoinSum,Description,Currency,TokenID)
    {
        ...
        let FromData = global.ACCOUNTS.ReadStateTR(FromID);
        if(!FromData)
        {
        ...
        return global.ACCOUNTS.SendMoneyTR(RunContext.Block, FromID, ToID, CoinSum, RunContext.BlockNum, RunContext.TrNum, Description, Description,   1, 1, 0, Currency,TokenID);
    }
    ...
    ListF.$Event = function (Description,Mode)
    {
        ...
        global.SMARTS.SendSmartEvent({Description:Description, Smart:RunContext.Smart.Num, Account:RunContext.Account.Num, BlockNum:RunContext.BlockNum, TrNum:RunContext.TrNum});
    ...
    ListF.$ReadAccount = function (ID)
    {
        DO(900);
        ID = ParseNum(ID);
        let Account = global.ACCOUNTS.ReadStateTR(ID);
        if(!Account)
    ...
    ListF.$ReadSmart = function (ID)
    {
        ...
        DO(900);
        ID = ParseNum(ID);
        let Smart = global.SMARTS.ReadSmart(ID);
        if(!Smart)
    ...
    ListF.$ReadState = function (ID)
    {
        ...
        {
            DO(100);
            let Smart = global.SMARTS.ReadSmart(Account.Value.Smart);
            if(!Smart)
            {
    ...
    ListF.$WriteState = function (Obj,ID)
    {
        ...
        global.ACCOUNTS.WriteStateTR(Account, RunContext.BlockNum, RunContext.TrNum);
    }

    ListF.$GetMaxAccount = function ()
    {
        DO(20);
        let DBChanges = GET_TR_CHANGES();
    ...
    ListF.$require = function (SmartNum)
    {
        ...
        let Smart = global.SMARTS.ReadSmart(SmartNum);
        if(!Smart)
        {
            throw "Smart does not exist. Error id number: " + SmartNum;
        }

        let EvalContext = GetSmartEvalContext(Smart);
    ...
    ListF.$ReadValue = function (Key,Format,IDFrom)
    {
        ...
        DO(500);
        let ID;
        let bNewVer=GETVERSION(RunContext.BlockNum)>=2?1:0;
        if(IDFrom && bNewVer)
            ID = +IDFrom;
        else
            ID = RunContext.Smart.Account;

        return global.ACCOUNTS.ReadValue(ID, Key, Format,!bNewVer);
    };
    ...
    ListF.$WriteValue = function (Key,Value,Format)
    {
        ...
        let ID = RunContext.Smart.Account;
        return global.ACCOUNTS.WriteValue(ID, Key, Value, Format, RunContext.BlockNum);
    };
    ...
    ListF.$RemoveValue = function (Key)
    {
        ...
        DO(1000);
        let ID = RunContext.Smart.Account;
        return global.ACCOUNTS.RemoveValue(ID, Key, RunContext.BlockNum);
    };
...
//Serialize Lib
    ...
    ListF2.$GetObjectFromBuffer = function (ValueBuf,Format)
    {
        ...
        return  global.SerializeLib.GetObjectFromBuffer(ValueBuf, Format, {});
    };
    ...
    ListF2.$GetBufferFromObject = function (Value,Format)
    {
        ...
        return global.SerializeLib.GetBufferFromObject(Value, Format, {})
    };
...
//New soft token support(like ERC)
    ...
    ListF2.$MoveCall = function (FromID,ToID,CoinSum,Description,Currency,TokenID,Method,Params,ParamsArr)
    {
        ...
        DO(3000);
        let CoinSumTo=DoTransfer(FromID,ToID,CoinSum,Description,Currency,TokenID);
        if(!CoinSumTo || CoinSumTo.SumCOIN===undefined || CoinSumTo.SumCENT===undefined)
            CoinSumTo=CoinSum;

        let Account = global.ACCOUNTS.ReadStateTR(ToID);
        let Smart=Account.Value.Smart;
        let PayContext = {FromID:FromID, ToID:ToID, Description:Description, Value:CoinSumTo, Currency:Currency,TokenID:TokenID, SmartMode:1};
    ...
    ListF2.$GetBalance = function (Account,Currency,ID)
    {
        ...
        if(!Currency)
        {
            let Data = global.ACCOUNTS.ReadStateTR(Account);
            if(!Data)
    ...
    ListF2.$RegInWallet = function (Account)
    {
        ...
        let SysCore=SYSCORE.GetInfo(RunContext.BlockNum);

        DO(100);

        let Ret=global.ACCOUNTS.RegInWallet(RunContext.BlockNum,Account,RunContext.Smart.Num);
        if(Ret)
        {
            DO(900);
            if(Ret===2)
    ...
    ListF2.$CreateSmart = function (FromSmart,Params)
    {
        ...
        return global.SMARTS.CreateSmartByParent(RunContext.Block, RunContext.BlockNum, RunContext.TrNum, FromSmart,Params);
    ...
    function GetParsing(Str)
    {
        ...
        let Code = LexerJS.stream;
        for(let key in LexerJS.FunctionMap)
        {
            Code += ";\nfunclist." + key + "=" + LOC_ADD_NAME + key;
        }
        for(let key in LexerJS.ExternMap)
        {
            let nType = LexerJS.ExternMap[key];
            if(nType === 1)
    ...
    {
        let Map = global.SMARTS.GetSmartMap();
        let EvalContext = Map["EVAL" + Smart.Num];
    ...
    function CreateSmartEvalContext(Code,Name,Version)
    {
        ...
        for(let key in EvalContext.funclist)
        {
            Object.freeze(EvalContext.funclist[key]);
        }
        for(let key in EvalContext)
        {
    ...
    function RunSmartMethod(Block,TR,SmartOrSmartID,Account,BlockNum,TrNum,PayContext,MethodName,Params,ParamsArr,nPublic,bMustHave, SmartCode)
    {
        ...
        let Smart = SmartOrSmartID;
        if(typeof SmartOrSmartID === "number")
        {
            Smart = global.SMARTS.ReadSmart(SmartOrSmartID);
            if(!Smart)
            {
                if(bMustHave)
        ...
        let context = GetSmartContext(Block, BlockNum, PayContext, TR, TrNum, Account, Smart, nPublic);

        let LocalRunContext = {Block:Block, Smart:Smart, Account:Account, BlockNum:BlockNum, TrNum:TrNum, Iteration:TR ? TR.Iteration : 0,
            context:context};

        if(!LocalRunContext.Iteration)
            LocalRunContext.Iteration = 0;
        let _RunContext = RunContext;
    ...
    function RunStaticSmartMethod(AccountNum,MethodName,Params,ParamsArr,nPublic)
    {
        ...
        if(typeof AccountNum==="number")
            Account = global.ACCOUNTS.ReadStateTR(AccountNum);
        else
            Account = AccountNum;

        if(!Account)
        {
            global.CLEAR_ALL_TR_BUFFER();
        ...
        try
        {
            ...
            let RetValue = RunSmartMethod(glBlock0, undefined, Account.Value.Smart, Account, BlockNum, 0, undefined, MethodName, Params,ParamsArr, nPublic);
    ...
    {
        DO(100);
        let Smart = global.SMARTS.ReadSmart(Num);
        return GET_SMART(Smart);
    }
    function StaticGetMaxAccountNum()
    {
        DO(100);
        return global.ACCOUNTS.GetMaxAccount();
    }
