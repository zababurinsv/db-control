...
debug('[(CWalletApp)a]', global.PROCESS_NAME)
    ...
    {
        Start() {
            global.CheckCreateDir(global.GetDataPath(this.GetWalletPath()))
        ...
        Init()
        {
            ...
            var Params = global.LoadParams(FName, undefined);
            debug('CWalletApp[(Init)a]',{
                FName: FName,
                Params: Params,
                "->": {
                    "0": "GetConfigFileName",
        ...
        {
            if(!arr1 || !arr2)
                global.ToLogTrace("Error arr for xor")
            var arr3 = [];
        ...
        SaveWallet()
        {
            ...
            var FName=this.GetConfigFileName();
            global.SaveParams(FName, Params)
        }
        ...
        FindMyAccounts(bClean)
        {
            ...
            var HiddenMap = global.LoadParams(this.GetHiddenFileName(), {});

            if(bClean)
                this.AccountMap = {}
            return global.ACCOUNTS.FindAccounts([this.PubKeyArr], this.AccountMap, HiddenMap, 0);
        }
        ...
        GetConfigFileName() {
            return global.GetDataPath(this.GetWalletPath() + "/config.lst");
        }

        GetHiddenFileName() {
            return global.GetDataPath(this.GetWalletPath() + "/hidden.lst");
        }
    }
