import buffer from '/modules/database/safe-buffer/dist/index.js'
...
import crypto from '/modules/database/crypto-browserify/dist/index.js';
...
export default async (fs) => {
  ...
  global.fs = {
    ...
    readSync: (...args) => {
      debug(-2,'🥚[(/)readSync]', args)
      // fs.readSync
    },
    readFileSync: (path) => {
      debug(-2,'🥚[(/)readFileSync]', path)
      // fs.readFileSync,
    },
    unlinkSync: (path) => {
    ...
    writeSync: (stream, buffer, offset, length) => {
      ...
      //     return fs.writeSync(stream, string, null, string.length)
      //   } else {
      //     return fs.writeSync(stream, buffer, offset, length)
      //   }
      // }
