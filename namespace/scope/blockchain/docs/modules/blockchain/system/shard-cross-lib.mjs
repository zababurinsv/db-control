...
import shardCrossTr from './shard-cross-tr.mjs'
...
debug('[(self)a]', global.PROCESS_NAME)
    let ShardCrossTr = shardCrossTr(global)
    const WorkStructCrossMsg = {};

    class ShardCrossLib extends ShardCrossTr
    {
            ...
            {
                {
                    ...
                    if(Tx && Tx.body && Tx.body[0] === TYPE_TRANSACTION_VBLOCK)
                    {
                        if(!GetVerifyTransaction(Block, TxNum))
                            return undefined;
                        var VBlock = global.SerializeLib.GetObjectFromBuffer(Tx.body, FORMAT_VBLOCK, WorkStructVBlock);
                        return VBlock;
                    }
                }
            }
        ...
        {
            var Arr = this.GetScrollCrossList(global.SERVER.CrossReceive, start, count);
        ...
        GetMaxCrossInNum()
        {
            return global.SERVER.CrossReceive.GetMaxNum();
        }
    };

    global.GetDataHashMsg = function (Msg)
    {
        var Arr = global.SerializeLib.GetBufferFromObject(Msg, global.FORMAT_CROSS_MSG, WorkStructCrossMsg);
        var Hash = sha3(Arr);
        return Hash;
    }

    return ShardCrossLib;
