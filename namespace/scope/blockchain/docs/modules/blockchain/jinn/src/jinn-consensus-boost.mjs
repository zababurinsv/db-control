...
//Engine context
    ...
    function InitClass(Engine) {
        ...
        Engine.SendMaxContextTree = new global.CBlockCache(function (a, b) {
        ...
        Engine.StartSendMaxHash = function (BlockNum) {
            ...
            for (var i = 0; i < Engine.LevelArr.length; i++) {
                ...
                if (Child && Child.IsHotReady()) {
                    Engine.SendMaxHashToOneNode(Context, BlockNum, Child, global.MAX_ITERATION_MAX_HASH);
                }
            }
        };
        ...
        Engine.GetMaxArrForSend = function (Context, Child, BlockNum, bNextSend) {
            ...
            if (!Child.SendMaxTimeCache)
                Child.SendMaxTimeCache = new global.CTimeCache(FMaxTreeCompare);
            ...
            for (var n = 0; n < Store.LiderArr.length; n++) {
                ...
                var Element = {
                    BlockNum: BlockNum,
                    Mode: 0,
                    DataHash: NodeStatus.DataHash,
                    LinkSumHash: NodeStatus.LinkSumHash,
                    TreeHash: NodeStatus.TreeHash,
                    SysTreeHash: NodeStatus.SysTreeHash,
                    MinerHash: NodeStatus.MinerHash,
                ...
                if (NodeStatus.LoadNum) {
                    ...
                    Element.Mode = 1;
                    Element.CountItem = Math.max(1, Math.min(global.JINN_CONST.MAX_ITEMS_FOR_LOAD, DeltaBlockNum));
                    Element.LoadN = NodeStatus.LoadNum;
                    Element.LoadH = NodeStatus.LoadHash;
                } else if (NodeStatus.LoadTreeNum) {
                    ...
                    Element.Mode = 0;
                    Element.LoadN = 0;
                    Element.LoadH = global.ZERO_ARR_32;
                    Element.CountItem = 0;
                }

                global.JINN_STAT.MaxSendCountItem = Math.max(global.JINN_STAT.MaxSendCountItem, Element.CountItem);

                if (Element.Mode === 2 && Context.BodyFromID !== Child.Level && DeltaBlockNum >= 8) {

                    Element.CountItem = 0;
                } else if (!bNextSend && Element.Mode && Element.CountItem) {
                    Element.CountItem = 1;
                }
                if (global.glUseMaxCache && (bNextSend || Element.Mode === 0)) {
                    ...
                    var Element2 = {};
                    global.CopyObjKeys(Element2, Element);
        ...
        Engine.SendMaxHashToOneNode = function (Context, BlockNum, Child, IterationNum, bNext) {
            ...
            if (BlockNum < global.JINN_CONST.BLOCK_GENESIS_COUNT - 1)
                return;
            ...
            for (var i = 0; i < Arr.length; i++) {
                ...
                if (Item.LoadH && !global.IsZeroArr(Item.LoadH))
                    ArrNeedHash.push(Item.LoadH);
            }
            global.JINN_STAT.MaxReqAll += MaxReqAll;

            global.JINN_STAT.MaxIteration = Math.max(global.JINN_STAT.MaxIteration, 1 + global.MAX_ITERATION_MAX_HASH - IterationNum);
            ...
            Engine.Send("MAXHASH", Child, {
                BlockNum: BlockNum,
                Receive: Engine.MaxHashReceiveCount,
                CodeVersionNum: global.CODE_VERSION.VersionNum,
                NetConstVer: global.JINN_NET_CONSTANT.NetConstVer,
                DepricatedArr: [],
                ArrRepeat: ArrRepeat,
                Debug: global.TEST_CONNECTOR,
                Arr: Arr,
                ...
                global.JINN_STAT.MaxLoadAll += Data.HeaderArr.length + Data.BodyArr.length;

                Child.LastTransferTime = Date.now();
                Child.TransferCount++;
                Child.DeltaTransfer = Child.LastTransferTime - SendTransferTime;

                if (!global.JINN_STAT.MinDTransfer || global.JINN_STAT.MinDTransfer > Child.DeltaTransfer)
                    global.JINN_STAT.MinDTransfer = (Child.DeltaTransfer < 1 ? 1 : Child.DeltaTransfer);
                global.JINN_STAT.MaxDTransfer = Math.max(global.JINN_STAT.MaxDTransfer, Child.DeltaTransfer);
                var Store = Engine.GetLiderArrAtNum(BlockNum);
                if (!Store)
                    return;
                ...
                for (var n = 0; n < Data.HeaderArr.length; n++) {
                    ...
                    if (Value) {
                        ...
                        if (!Store.HeaderLoad)
                            Store.HeaderLoad = 0;
                        Store.HeaderLoad++;
                        global.JINN_STAT.HeaderLoad++;
                        global.JINN_STAT.MaxLoad++;
                ...
                global.JINN_STAT.MaxHeaderLoad = Math.max(global.JINN_STAT.MaxHeaderLoad, Data.HeaderArr.length);
                ...
                if (Data.BodyTreeNum) {
                    ...
                    if (LID && !Context.BodyTreeNum) {
                        for (var d = -global.JINN_CONST.MAX_DELTA_PROCESSING; d <= global.JINN_CONST.MAX_DELTA_PROCESSING; d++) {
        ...
        Engine.MAXHASH_SEND = {
            Receive: "uint", BlockNum: "uint32", NetConstVer: "uint32", CodeVersionNum: "uint32", DepricatedArr: [{
                Mode: "byte",
                DataHashNum: "byte",
                DataHash: "zhash",
                MinerHash: "hash",
        ...
        Engine.MAXHASH_RET = {
            result: "byte", errnum: "byte", result2: "byte", Reserve: "uint32", Mode: "byte", DepricatedHeaderArr: [{
                BlockNum: "uint32",
                PrevSumPow: "uint", LinkSumHash: "hash", TreeHash: "zhash", MinerHash: "hash", OldPrevHash8: "zhash"
            }], Reserve2: "uint32", BodyArr: [{
                BlockNum: "uint32",
                TreeHash: "hash",
                ArrFull: [{body: "tr"}],
                ArrTtTx: [{HashTicket: "arr" + global.JINN_CONST.TT_TICKET_HASH_LENGTH, body: "tr"}],
            }], BodyTreeNum: "uint32",
            BodyTreeHash: "zhash", HeaderArr: [{
                BlockNum: "uint32", PrevSumPow: "uint", LinkSumHash: "hash", SysTreeHash: "zhash", TreeHash: "zhash",
                MinerHash: "hash", OldPrevHash8: "zhash", Reserve: "uint32"
            }], CurTime: "uint32",
        };
        ...
        Engine.DoMaxHash = function (Child, Data) {
            var WasReads = global.JINN_STAT.ReadRowsDB;
            ...
            if (Data.Arr.length > global.JINN_CONST.MAX_LEADER_COUNT)
                Data.Arr.length = global.JINN_CONST.MAX_LEADER_COUNT;
            ...
            var Was = 0;
            var BodyTreeHash = global.ZERO_ARR_32;
            var BodyTreeNum = 0;
            ...
            for (var i = 0; i < Data.Arr.length; i++) {
                ...
                if (i > 0 && Data.BlockNum - Status.LoadN > global.JINN_CONST.MAX_DEPTH_FOR_SECONDARY_CHAIN)
                    continue;

                Status.CountItem = Math.min(global.JINN_CONST.MAX_ITEMS_FOR_LOAD, Status.CountItem);

                if (Status.Mode === 1) {
                    HeaderArr = Engine.GetHeaderArrForChild(Data.BlockNum, Status, Child);
                    {
                        global.JINN_STAT.WantHeader += Status.CountItem;
                        global.JINN_STAT.UploadHeader += HeaderArr.length;
                    }
                    if (HeaderArr.length) {
                        RetMode = 1;
                    }
                    ...
                    if (BodyArr.length) {
                        ...
                        {
                            global.JINN_STAT.WantBody += Status.CountItem;
                            global.JINN_STAT.UploadBody += BodyArr.length;
                        }
                    }
                }
            }
            ...
            return {
                ...
                DepricatedHeaderArr: [],
                BodyArr: BodyArr,
                BodyTreeNum: BodyTreeNum,
                BodyTreeHash: BodyTreeHash,
                CurTime: CurTime,
                HeaderArr: HeaderArr
            };
        };
        ...
        Engine.CheckPacketSize = function (BlockNum, BlockNumLoad, Size) {
            if (Size >= global.JINN_CONST.MAX_PACKET_SIZE_RET_DATA) {
                return 0;
            }
            return 1;
        };
        ...
        Engine.GetHeaderArrForChild = function (BlockNum, Status, Child) {
            ...
            if (TestHeader && MainItem && TestHeader.Position === MainItem.MainPosition) {
                for (var n = 0; n < CountItem; n++) {
                    ...
                    if (!BlockHeader || BlockHeader.BlockNum < global.JINN_CONST.BLOCK_GENESIS_COUNT - 1)
                        break;
        ...
        Engine.CheckProcessTime = function (Child) {
            ...
            if (deltaTime > global.JINN_CONST.MAX_CHILD_PROCESS_TIME) {
                global.JINN_STAT.BreakUploadTime++;
                return 0;
            } else
                return 1;
        };
        ...
        Engine.GetBodyArrForChild = function (Child, BlockNum, LoadNum, LoadHash, CountItem, bFirstBody) {
            ...
            if (TestHeader && MainItem && TestHeader.Position === MainItem.MainPosition) {
                for (var n = 0; true; n++) {
                    ...
                    if (Num <= 0)
                        break;
                    if (n >= global.JINN_CONST.MAX_ITEMS_FOR_LOAD * 2)
                        break;
            ...
            if (BlockBody) {
                ...
                for (var n = 1; n < true; n++) {
                    ...
                    if (!BlockBody.TreeHash || !BlockBody.TxData || global.IsZeroArr(BlockBody.TreeHash)) {
                        continue;
                    }
        ...
        Engine.AddBodyBlockToArr = function (Child, Arr, BlockBody, Size, BlockNum) {
            ...
            if (!BlockBody.TxData || !BlockBody.TxData.length) {
                global.ToLogTrace("Error send block = " + BlockBody.BlockNum + " - NO TX BODY  BlockBody.TreeHash=" + BlockBody.TreeHash);
                return Size;
            }
            ...
            global.JINN_STAT.BodyTxSend += BlockBody.TxData.length;

            var DeltaBlockNum = Engine.CurrentBlockNum - BlockNum;
            if (DeltaBlockNum < global.JINN_CONST.STEP_CLEAR_MEM && BlockBody.TxData.length && Engine.DB.SetTxDataCache) {
    ...
    function FMaxTreeCompare(Val1, Val2, NoNum) {
        ...
        var Comp1 = global.CompareArr(Val1.DataHash, Val2.DataHash);
        if (Comp1)
            return Comp1;
        var Comp2 = global.CompareArr(Val1.MinerHash, Val2.MinerHash);
        if (Comp2)
            return Comp2;

        var LoadH1 = Val1.LoadH;
        if (!LoadH1)
            LoadH1 = global.ZERO_ARR_32;
        var LoadH2 = Val2.LoadH;
        if (!LoadH2)
            LoadH2 = global.ZERO_ARR_32;

        return global.CompareArr(LoadH1, LoadH2);
    }
