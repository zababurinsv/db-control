...
debug('[(self)a]', global.PROCESS_NAME)
    class CDBRow extends global.CDBBase
    {
        ...
        Write(Data, RetBuf)
        {
            ...
            var BufWrite0 = global.SerializeLib.GetBufferFromObject(Data, this.Format, this.WorkStruct, 0);
            if(this.DataSize < BufWrite0.length)
            {
                global.ToLogTrace("Error max size = " + this.DataSize + " current = " + BufWrite0.length)
            }
            if(global.Buffer)
            {
