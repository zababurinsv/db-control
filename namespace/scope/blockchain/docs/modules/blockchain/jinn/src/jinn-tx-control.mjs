...
//Engine context
    ...
    function InitClass(Engine) {
        ...
        Engine.GetTopTxArray = function (Arr, BlockNum) {
            if (global.JINN_CONST.TEST_MODE_1)
                return Arr;
        ...
        Engine.FilterAndCheckSizeBlockTXArray = function (Block) {
            ...
            if (Arr.length > global.JINN_CONST.MAX_TRANSACTION_COUNT)
                Arr.length = global.JINN_CONST.MAX_TRANSACTION_COUNT;

            var BufLength = 0;
            for (var i = 0; i < Arr.length; i++) {
                var Item = Arr[i];
                BufLength += Item.body.length;
                if (BufLength > global.JINN_CONST.MAX_BLOCK_SIZE) {
                    Arr.length = i + 1;
                    break;
                }
            }
        ...
        Engine.IsValidateTx = function (Tx, StrCheckName, BlockNum, IsBlockTx) {
            ...
            if (!Tx || !Tx.IsTx || !Tx.body || Tx.body.length < 8 || Tx.body.length > global.JINN_CONST.MAX_TX_SIZE)
                return 0;
            var Result = global.CheckTx(StrCheckName, Tx, BlockNum, 1);
            if (!Result)
                global.JINN_STAT.NoValidateTx++;
