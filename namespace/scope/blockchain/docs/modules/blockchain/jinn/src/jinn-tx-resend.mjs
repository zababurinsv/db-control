...
//Engine context
    ...
    function InitClass(Engine) {
        ...
        Engine.DoResend = function () {
            ...
            if (!global.JINN_CONST.COUNT_RESEND)
                return;
            if (Engine.GetMaxNumBlockDB() < Engine.CurrentBlockNum - global.JINN_CONST.STEP_CLEAR_MEM * 2)
                return;

            var StartNum = Engine.CurrentBlockNum - global.JINN_CONST.STEP_CLEAR_MEM;
            var FinishNum = Engine.CurrentBlockNum - global.JINN_CONST.STEP_RESEND;
            ...
            var TreeDB = new global.RBTree(function (a, b) {
            ...
            for (var BlockNum = StartNum; BlockNum <= FinishNum; BlockNum++) {
                ...
                for (var t = 0; t < Arr.length; t++) {
                    ...
                    if (!TreeDB.find(Tx)) {
                        ...
                        if (Tx.ResendCount <= global.JINN_CONST.COUNT_RESEND)
                            ResendArr.push(Tx);
                    }
                }
            }
