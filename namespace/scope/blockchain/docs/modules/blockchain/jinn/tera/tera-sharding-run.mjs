    ...
    function Init(Engine)
    {
        ...
        Engine.CreateVBlock = function (ShardData,VBlockFirst)
        {
            ...
            if(MsgItem)
            {
                ...
                while(MsgItem && MsgItem.RowNum >= RowNum)
                {
                    ...
                    {
                        if(global.IsEqArr(MsgItem.RowHash, RowHash))
                        {
                            break;
                        }
                        else
                        {
                            ToLog("CreateVBlock: Error RowHash cross msg on RowNum=" + RowNum + " RowHash=" + global.GetHexFromArr8(RowHash) + "  MsgItem.RowHash=" + global.GetHexFromArr8(MsgItem.RowHash),
                                3);
                            return undefined;
                        }
                    }
                ...
                VBlockItem._Body = global.SerializeLib.GetBufferFromObject(VBlockItem, FORMAT_VBLOCK, WorkStructVBlock);
                VBlockItem.body = VBlockItem._Body;
                VBlockItem.Hash = global.sha3(VBlockItem._Body);
        ...
        Engine.DoSendingCrossArr = function (BlockNum)
        {
            if(!global.USE_MINING || !global.USE_MINING_SHARDS)
                return [];
            if(global.SERVER.CrossReceive.Ready === 100)
                return [];

            if(Engine.IsCorrectTxData() !== 1)
                return  - 2;

            if(!global.SERVER.CrossReceive.Ready)
                return  - 3;
        ...
        global.SERVER.DoCreateCrossArr = function (Block)
        {
            if(!global.USE_MINING_SHARDS)
                return [];
            ...
            for(var Shard in Engine.VBlockChannelList)
            {
                ...
                if(!LastHead || !global.IsEqArr(LastHead.RowHash, VBlockFirst.PrevRowHash))
                {
                    ToLog("HeadBlock was changed from: " + VBlockInfo(VBlockFirst) + " to: " + (LastHead ? global.GetHexFromArr8(LastHead.RowHash) : "-"),
                        3);
                    ShardData.VBlockArr.length = 0;
                    continue;
                }
