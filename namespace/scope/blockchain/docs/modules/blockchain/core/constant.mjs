"use strict";
import StartLib from './startlib.mjs'
import constantMode from './const-mode.mjs'
...
debug(-4, '[(self)a]', global.PROCESS_NAME)
    ...
    global.REGISTER_SYS_DAPP = function (App,Type) {
        let Name = App.Name();
        global.DApps[Name] = App;
        if(global.DAppByType[Type]) {
            throw "Error on registering app " + App.Name() + ": tx type = " + Type + " has already been registered in the app " + global.DAppByType[Type].Name();
        }
        global.DAppByType[Type] = App;
    };
    //~~~~~~~~~~~~~~~~~~~~~~~~
...
//old mode
    ...
    global.BLOCK_GENESIS_COUNT = global.BLOCK_PROCESSING_LENGTH2;
    ...
    Object.defineProperty(global, "ZERO_ARR_32", { configurable: true, writable:false, value: ZERO_ARR_32_0});
    Object.defineProperty(global, "MAX_ARR_32", { configurable: true, writable:false, value: MAX_ARR_32_0});
    ...
    {
        return global.SYSCORE.GetActive(BlockNum);
    };

    global.PRICE_DAO = function (BlockNum) {
        return global.SYSCORE.GetInfo(BlockNum);
    ...
    var ShardParamPath = global.GetDataPath("shard.mjs");
    debug('$$[(constant)ShardParamPath]', ShardParamPath)
    if(fs.existsSync(ShardParamPath)) {
        global.runcode(ShardParamPath);
    }else {
        constantMode(global)
    } //Shard init values
    global.CheckCreateDir(global.DATA_PATH);
    global.CheckCreateDir(global.DATA_PATH + "DB");
    if(!global.START_NETWORK_DATE) {
        global.START_NETWORK_DATE = global.FindBlockchainStartTime(1);
    }

    global.START_CODE_VERSION_NUM = global.UPDATE_CODE_VERSION_NUM;
    ...
    function InitParamsArg()
    {
        for(var i = 1; i < process.argv.length; i++)
        {
            ...
            if(STR.substr(0, 5) == "FROM:")
            {
                ...
                var LoadShardParams = Loader(global)
