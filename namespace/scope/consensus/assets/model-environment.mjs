...
//JINN_CONST.MAX_DELTA_PROCESSING=0;
    global.JINN_CONST.MAX_LEADER_COUNT=4;//<--------------------- влияет на трафик по умолчанию
...
//JINN_CONST.MAX_CONNECT_TIMEOUT=5;
    ...
    global.JINN_CONST.MIN_TIME_SEND_TT_PERIOD=200;
    global.JINN_CONST.MAX_TIME_SEND_TT_PERIOD=500;
    global.JINN_CONST.DELTA_TIME_NEW_BLOCK=200;//иначе не успеваем записать собственные намайненные блоки


    global.JINN_CONST.TEST_DIV_TIMING_HASH=1;
//сеть-ноды



    global.JINN_CONST.MAX_LEVEL_CONNECTION=6;
    global.JINN_CONST.EXTRA_SLOTS_COUNT=4;
...
//JINN_CONST.MAX_CONNECT_COUNT=20;
    ...
    global.CreateModel=function (Count,bNoCreate)
    {
        ...
        if(global.IP_VERSION===6)
            global.InitNodeID(NodeRoot,"::",20000);
        else
            global.InitNodeID(NodeRoot,"127.0.0.1",20000);

        Env.NetNodeMap[NodeRoot.IDStr]=NodeRoot;
        global.JINN_EXTERN.NodeRoot=NodeRoot;
    ...
    function CTeraEnvironment(Count)
    {
        ...
        Env.DoRun=function()
        {
            ...
            NextRunEngine(global.JINN_EXTERN.NodeRoot);
        ...
        {
            Env.TickCount++;
            Env.CurrentBlockNum = global.JINN_CONST.START_BLOCK_NUM+Math.floor(Env.TickCount/10);
        };
        ...
        Env.NextTickBlockNum0=function()
        {
            ...
            {
                Env.CurrentBlockNum = global.JINN_CONST.START_BLOCK_NUM;
                Env.CurrentBlockNumTime=Time;
            }

            if(Time-Env.CurrentBlockNumTime >= global.JINN_CONST.CONSENSUS_PERIOD_TIME)
            {
                //next num
                Env.CurrentBlockNum++;
                Env.CurrentBlockNumTime=Time;
        ...
        global.JINN_EXTERN.GetCurrentBlockNumByTime=Env.GetCurrentBlockNumByTime;
        //JINN_EXTERN.NetNodeArr=Env.NetNodeArr;
        global.JINN_EXTERN.GetNodeByAddr=Env.GetNodeByAddr;

        global.JINN_EXTERN.GetCurrentTickNum=function() {
        ...
        Env.CalcMaxConsensusHash=function() {
            ...
            var Map={};
            var BlockNum = global.JINN_EXTERN.GetCurrentBlockNumByTime() - global.JINN_CONST.STEP_LAST;
