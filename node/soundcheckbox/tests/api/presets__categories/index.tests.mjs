...
export default () => {
    describe(`Positive`, function() {
        ...
        it('create preset categories', async () => {
            try {
                const data = await req.post('http://localhost:4552/api/presets/categories', { answer: 42 })
        ...
        it('get preset categories', async () => {
            try {
                const data = await req.get('http://localhost:4552/api/presets/categories');
                console.log('DATA', data);
                return  true;
            } catch (error) {
