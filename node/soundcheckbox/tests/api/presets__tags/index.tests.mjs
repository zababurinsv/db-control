...
export default () => {
    describe(`Positive`, function() {
        ...
        it('create preset tags', async () => {
            try {
                const data = await req.post('http://localhost:4552/api/presets/tags', { answer: 42 })
        ...
        it('get preset tags', async () => {
            try {
                const data = await req.get('http://localhost:4552/api/presets/tags');
                console.log('DATA', data);
                return  true;
            } catch (error) {
