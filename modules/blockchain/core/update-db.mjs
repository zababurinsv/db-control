...
debug('[(self)a]', global.PROCESS_NAME)
    ...
    function RunOnUpdate()
    {
        ...
        var UpdateInfo = global.LoadParams(fname, {UPDATE_NUM_COMPLETE:2000, JINN_MODE_VER2:1});
        ...
        if(CurNum !== global.UPDATE_CODE_VERSION_NUM)
        {
            ...
            global.ToLog("---------- UPDATER Start from:" + CurNum);
        ...
        var FName2=global.GetDataPath("DB/smart2");
        if(!fs.existsSync(FName2) && global.SMARTS.GetMaxNum()>0)
            UpdateSmartDB();
    ...
    function DeleteOldDBFiles()
    {
        setTimeout(function ()
        {
            global.ToLog("---------- UPD: START DeleteOldDBFiles");
    ...
    {
        global.ToLog("---------- UPD: Prepare RewriteAllTransactions");
        setTimeout(function ()
        {
    ...
    function SendRewrteTx(StartNum)
    {
        setTimeout(function ()
        {
            global.ToLog("---------- UPD: START RewrteTx from: " + StartNum);
            if(global.TX_PROCESS && global.TX_PROCESS.RunRPC)
    ...
    {
        var AccountsHash = global.ACCOUNTS.GetHashOrUndefined(BlockNum);
        if(!AccountsHash)
        {
            if(bMust)
                return 0;
        ...
        if(global.GetHexFromArr(AccountsHash) === StrHash)
            return 1;
        else
            return 0;
    }
    ...
    {
        var DBAct = global.ACCOUNTS[Name];
        ...
        while(1)
        {
            ...
            if(Item.Mode === 200)
            {
                ...
                if(Item)
                {
                    var Block = global.SERVER.ReadBlockHeaderDB(Item.BlockNum);
                    if(!Block || !global.IsEqArr(Block.SumHash, Item.HashData.SumHash))
                    {
                        global.ToLog("---------Error SumHash on BlockNum=" + Item.BlockNum);
            ...
            Num++;
            Num % 100000 === 0 && global.ToLog(Name + ": Check " + Num);
        }
    }
