...
debug(-4, '[(self)a]', global.PROCESS_NAME)
    ...
    var file_name_log_tx = global.GetDataPath("log-tx.log");
    var file_name_log_txPrev = global.GetDataPath("log-tx-prev.log");
    ...
    global.ToLog = function (Str,Level,bNoFile) {
        ...
        if(Level > 50)
            global.ToLogTrace("Err Log Level=" + Level);

        if(!Level || Level <= global.LOG_LEVEL)
        {
            global.ToLogClient(Str, undefined, undefined, bNoFile);
        }
    ...
    function ToLogClient(Str,StrKey,bFinal,bNoFile,bToWeb,BlockNum,TrNum) {
        ...
        if(global.PROCESS_NAME === "MAIN")
        {
            if(!StrKey)
                StrKey = "";
            AddToArrClient(Str, StrKey, bFinal, global.GetStrOnlyTime(),BlockNum,TrNum);
        }
        else
        {
    ...
    function AddToArrClient(text,key,final,time,BlockNum,TrNum)
    {
        ...
        global.ArrLogClient.push({id:global.ArrLogCounter, text:text, key:key, final:final, time:time,BlockNum:BlockNum,TrNum:TrNum});
    ...
    global.ToLogStack = function (StrError,Data,LogLevel)
    {
        ...
        global.ToLog("" + StrError + ":" + Data, LogLevel);
    }

    global.ToLogTrace = function (Str,LogLevel)
    {
        global.ToLogStack(Str, new Error().stack, LogLevel);
    }

    global.StopAndExit = function (Str)
    {
        global.ToLogStack(Str, new Error().stack, 0);
    ...
    global.ToLogOne = function (Str,Str2,Level)
    {
        if(!MapLogOne[Str])
        {
            MapLogOne[Str] = 1;
            if(Str2)
                global.ToLog(Str + Str2, Level);
            else
                global.ToLog(Str, Level);
    ...
    function ToLogFile(file_name,Str,bNoFile,bNoSend)
    {
        ...
        if(global.PROCESS_NAME) {
            console.log("" + global.JINN_PORT + ": " + global.GetStrOnlyTime() + ": " + Str);
        } else {
            console.log(Str);
        }
    ...
    function SaveToLogFileSync(fname,Str) {
        try {
            debug(-2, '[(log*)SaveToLogFileSync]',fname,Str)
            let StrLog = global.GetStrTime() + " : " + Str + "\r\n";
            let file_handle = fs.openSync(fname, "a");
    ...
    function CheckSizeLogFile(FILE_NAME_LOG,FILE_NAME_LOG_PREV)
    {
        ...
        setInterval(function ()
        {
            fs.stat(FILE_NAME_LOG, function (err,stat) {
                ...
                {
                    console.warn('надо проверить работоспособность на браузере')
