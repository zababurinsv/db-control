...
let debug = (maxCount, id, props, data, ...args) => {
    ...
    logs.assert(-14, url, id, props, data, ...args)
}
...
//Engine context
    ...
    function InitClass(Engine) {
        ...
        Engine.InHotStart = function (Item) {
            ...
            if (HotItem.HotStart && Date.now() - HotItem.HotStart <= global.MAX_HOT_CONNECTION_DELAY)
                return 1;
            else
                return 0;
        };
        ...
        Engine.InHotDeny = function (Item) {
            ...
            var HotItem = Item.HotItem;
            if (HotItem.DenyHotStart && Date.now() - HotItem.DenyHotStart <= global.MAX_DENY_HOT_CONNECTION)
                return 1;
            else
                return 0;
        };
        ...
        Engine.TryHotConnection = function (Child, bSend) {
            debug(-3, '[(TryHotConnection*)SendConnectReq]', Child)
            if (Engine.ROOT_NODE) {
                return 0;
            }
        ...
        Engine.SetHotConnection = function (Child, bCheckLevel) {
            ...
            var LevelArr = Engine.GetLevelArr(Child);
            LevelArr[Child.Level] = Child;
            Child.ToLogNet("SetHotConnection Level: " + Child.Level + (Child.Level >= global.JINN_CONST.MAX_LEVEL_CONNECTION ? " (Extra slot)" : ""));
        ...
        Engine.CONNECTLEVEL = function (Child, Data) {
            ...
            if (WasChild && (Date.now() - WasChild.ConnectStart) > global.JINN_CONST.RECONNECT_MIN_TIME * 1000 && WasChild.Score * 2 < Child.Score) {
            ...
            if (!Ret && Data.UseExtraSlot) {
                for (var i = global.JINN_CONST.MAX_LEVEL_CONNECTION; i < global.JINN_CONST.MAX_LEVEL_ALL(); i++) {
                    var Slot = LevelArr[i];
                    if (!Slot) {
                        WasExtra = 1;
                        Child.Level = i;
            ...
            if (0 && !Ret && !WasExtra) {
                ...
                if (MinChild && Date.now() - MinChild.ConnectStart > global.JINN_CONST.RECONNECT_MIN_TIME * 1000) {
                    if ((MinChild.Level === Child.Level || MinChild.Level >= global.JINN_CONST.MAX_LEVEL_CONNECTION) && MinChild.Score < Child.Score) {
        ...
        Engine.AddrLevelArr = function (arr1, arr2, bLimit) {
            ...
            if (bLimit && Level >= global.JINN_CONST.MAX_LEVEL_CONNECTION)
                Level = global.JINN_CONST.MAX_LEVEL_CONNECTION - 1;
        ...
        Engine.DoConnectHotLevels = function () {
            ...
            for (var L = 0; L < global.JINN_CONST.MAX_LEVEL_CONNECTION; L++) {
                ...
                function LevelConnect(HotChild, LevelData) {
                    for (var i = 0; i < LevelData.Connect.length; i++) {
                        ...
                        if (!global.CanTime(Item, "SendHotConnect", 1000, 1.5)) {
                ...
                function LevelCheck(HotChild) {
                    if (HotChild) {
                        ...
                        if (Item.SendHotConnectPeriod !== 1000) {
                            global.ResetTimePeriod(HotChild.AddrItem, "SendHotConnect", 1000, 20000);
                        }
                    }
                };
            }
        ...
        Engine.CheckHotConnections = function (LevelArr, bCross) {
            for (var L = 0; L < global.JINN_CONST.MAX_LEVEL_ALL(); L++) {
                ...
                if (bCross && !Child.IsCluster) {
                    bNeedDisconnect = 1;
                    StrError = "5# Disconnect not clustered node " + global.ChildName(Child);
                } else if (bCross && Child.ShardName === global.JINN_CONST.SHARD_NAME) {
                    bNeedDisconnect = 1;
                    StrError = "6# Disconnect cross hot node " + global.ChildName(Child);
                } else if (!Engine.InHotStart(Child) && (!Child.IsOpen() || Child.Del)) {
                    bNeedDisconnect = 1;
                    StrError = "1# Connect not was open " + global.ChildName(Child);
                } else if (Child.HotStart && !Engine.InHotStart(Child)) {
                    bNeedDisconnect = 1;
                    StrError = "2# Bad connect time period " + global.ChildName(Child);
                } else if (Child.FirstTransferTime) {
                    ...
                    if ((Delta1 > START_TRANSFER_TIMEOUT) && (Delta2 > MAX_TRANSFER_TIMEOUT)) {
                        bNeedDisconnect = 1;
                        StrError = "3# Not data exchange a long time " + global.ChildName(Child);
                ...
                if (Child.IsHot() && !Child.IsHotReady()) {
                    bNeedDisconnect = 1;
                    StrError = "4# CheckHotConnections - not HotReady " + global.ChildName(Child);
                }
