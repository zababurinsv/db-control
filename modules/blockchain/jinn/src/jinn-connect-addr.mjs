...
debug('[(self)a]', global.PROCESS_NAME)
    ...
    function InitClass(Engine)
    {
        ...
        Engine.NodesTree = new global.RBTree(FNodeAddr);
        ...
        {
            if(!global.CanTime(Child, "SendGetNodesReq", 1000))
                return;

            if(!Child.Iterator)
                Child.Iterator = {Level:0, Arr:[]};

            Engine.Send("GETNODES", Child, {Iterator:Child.Iterator, Version:global.GETNODES_VERSION}, function (Child,Data)
            {
                if(!Data || Data.Version < global.GETNODES_VERSION)
                    return;
        ...
        Engine.GETNODES = function (Child,Data)
        {
            ...
            if(Engine.DirectIP && !Engine.ROOT_NODE)
                if(global.IsZeroArr(Data.Iterator))
                {
                    if(Engine.AddrItem)
                    {
            ...
            for(var i = 0; i < global.JINN_CONST.MAX_RET_NODE_LIST; i++)
            {
                var Item = Engine.GetNextNodeAddr(Data.Iterator, 0);
                if(Item && !Item.IsLocal)
                {
                    if(Child.IsCluster || Item.ShardName === global.JINN_CONST.SHARD_NAME)
                        Arr.push(Item);
                }
            }

            return {Version:global.GETNODES_VERSION, Arr:Arr, Iterator:Data.Iterator};
        };
        ...
        Engine.AddNodeAddr = function (AddrItem,FromChild)
        {
            ...
            AddrItem.IDArr = global.CalcIDArr(AddrItem.ip, AddrItem.port);
            ...
            if(global.JINN_EXTERN.NodeRoot && AddrItem.ip === global.JINN_EXTERN.NodeRoot.ip && AddrItem.port === global.JINN_EXTERN.NodeRoot.port)
        ...
        {
            var Level = Iterator.Level % global.JINN_CONST.MAX_LEVEL_CONNECTION;
        ...
        Engine.SetParamsNodeAddr = function (WasItem,NewItem)
        {
            if(!WasItem.BlockNum || WasItem.BlockNum < NewItem.BlockNum)
            {
                ...
                if(!WasItem.AddrHashPOW)
                    WasItem.AddrHashPOW = global.MAX_ARR_32;
                if(!WasItem.BlockNum)
                    WasItem.BlockNum = 0;
        ...
        Engine.GetAddrPower = function (AddrHashPOW,BlockNum) {
            debug(-3, '------------♉[(GetAddrPower*) AddrHashPOW BlockNum]', {
                AddrHashPOW: AddrHashPOW,
                BlockNum: BlockNum
            })
            if(!AddrHashPOW) {
                debug(-3, '------------♉[(GetAddrPower*) Power]', "0")
                return 0;
            }
            ...
            DeltaNum += 1000;
            var Power = 1000 * (global.GetPowPower(AddrHashPOW)) / DeltaNum;

            debug(-3, '------------♉[(GetAddrPower*) Power]', Power, DeltaNum)
            return Power;
        };
        ...
        Engine.SetOwnIP = function (ip)
        {
            ...
            var NewIDArr = global.CalcIDArr(Engine.ip, Engine.port);
            if(!global.IsEqArr(Engine.IDArr, NewIDArr))
            {
        ...
        Engine.RecreateLevelsArray = function ()
        {
            if(Engine.NodesTree.size)
            {
                ...
                while((Item = it.next()) !== null)
                {
                    ...
                    if(Item.Level < global.JINN_CONST.MAX_LEVEL_CONNECTION)
                    {
                ...
                for(var n = 0; n < global.JINN_CONST.MAX_LEVEL_CONNECTION; n++)
                {
        ...
        Engine.CaclNextAddrHashPOW = function (Count)
        {
            ...
            var MaxAddrHashPOW = global.MAX_ARR_32;
            var MaxNonce = 0;
            ...
            for(var i = 0; i < Count; i++)
            {
                ...
                if(global.CompareArr(Hash, MaxAddrHashPOW) < 0)
                {
                    MaxAddrHashPOW = Hash;
        ...
        Engine.IsCorrectNode = function (ip,port)
        {
            ...
            if(global.JINN_CONST.UNIQUE_IP_MODE)
            {
                var CountPorts = Engine.GetCountPortsByIP(ip);
                if(CountPorts >= global.JINN_CONST.UNIQUE_IP_MODE)
                    return 0;
            }
            return 1;
        };
        ...
        Engine.FindAddrItemByArr = function (IDArr)
        {
            ...
            while((Item = it.next()) !== null)
            {
                if(Item.IDArr && global.IsEqArr(Item.IDArr, IDArr))
                    return Item;
            }
            return undefined;
        };
        ...
        Engine.SetAddrItemForChild = function (AddrChild,Child,bAddNode) {
            debug(-3, '---------------♉[(SetAddrItemForChild*) ip]', AddrChild.ip)
            if(!Child.AddrItem) {
                let FindItem = Engine.NodesTree.find(AddrChild);
                if(!FindItem) {
                    FindItem = AddrChild;
                    if(bAddNode) {
                        Child.DirectIP = 1;
...
//Engine context
    ...
    function GetHashFromNum2(Value1,Value2)
    {
        ...
        global.WriteUintToArrOnPos(MeshArr, Value1, 0);
        global.WriteUintToArrOnPos(MeshArr, Value2, 6);

        return global.sha3(MeshArr, 1);
    }
    function GetHashFromArrNum2(Arr,Value1,Value2)
    {
        ...
        global.WriteUintToArrOnPos(MeshArr, Value1, 32);
        global.WriteUintToArrOnPos(MeshArr, Value2, 38);

        return global.sha3(MeshArr, 2);
    ...
    function CalcIDArr(ip,port) {
        ...
        var IDArr = global.sha3(HostName, 3);
        return IDArr;
    }
