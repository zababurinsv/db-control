...
let debug = (maxCount, system, property, substrate, ...args) => {
    ...
    // logs.assert(-8 ,url, system, property, substrate, args)
}
...
//Engine context
    function DoNode(Engine) {
       debug('[(DoNode*)Engine]')
        ...
        if (global.UPDATE_CODE_SHARDING && global.GetCurrentBlockNumByTime() < global.UPDATE_CODE_SHARDING) {
            return;
        }
        Engine.SendCrossTx();
    }
    ...
    function InitClass(Engine) {
        debug('[(InitClass*)Engine]')

        Engine.CrossLevelArr = [];

        Engine.CanShardConnect = function (Child, ShardName) {
            debug('[(CanShardConnect*)ShardName]', ShardName)

            if (!Child.IsCluster && ShardName !== global.JINN_CONST.SHARD_NAME) {
                return 0;
            } else {
                return 1;
            }
        ...
        Engine.SetShardName = function (Child, ShardName) {
            debug('[(SetShardName*)ShardName]', ShardName)
            if (!ShardName) {
                ShardName = global.JINN_CONST.SHARD_NAME;
            }
        ...
        Engine.GetLevelArr = function (Child) {
            if (Child.IsCluster && Child.ShardName !== global.JINN_CONST.SHARD_NAME) {
        ...
        Engine.CROSSMSG_SEND = {
            Reserve: "uint",
            ChannelArr: [{ChannelName: "str20", Reserve: "uint", RowNum: "uint", RowHash: "hash"}]
        };
        Engine.CROSSMSG_RET = {
            Reserve: "uint", result: "byte", ChannelArr: [{
                ChannelName: "str20", result: "byte", MaxRowNum: "uint", Arr: [{
                    BlockNum: "uint32",
