...
//Engine context
    ...
    function InitClass(Engine) {
        ...
        Engine.GetStrFromJumpArr = function (ArrJump) {
            ...
            for (var i = 0; i < ArrJump.length; i++) {
                ...
                if (CurBlock) {
                    ...
                    Str2 += global.GetHexFromArr(CurBlock.SumHash).substr(0, 6);
                    bWas = 1;
                }
            }
            return Str + " (" + Str2 + ")";
        ...
        Engine.GetFirstHeadBlock = function (BlockSeed) {
            ...
            while (true) {
                ...
                if (!global.IsEqArr(Block.PrevSumHash, PrevBlock.SumHash))
                    global.ToLogTrace("Error PrevSumHash on Block=" + Block.BlockNum);
            ...
            if (global.TEST_BLOCK_LIST) {
                ...
                if (Block && Block0 && Block0 !== Block && !global.IsEqArr(Block0.SumHash, Block.SumHash) && Block.BlockNum > Block0.BlockNum) {
            ...
            if (Delta > 0 && Count >= 2) {
                ...
                if (Count > 2000)
                    Engine.ToLog("[" + Count + "/" + nSet + "]" + "  jump: " + Delta + "   " + BlockSeed0.BlockNum + "->" + Block.BlockNum + "   " + global.GetHexFromArr(BlockSeed0.SumHash).substr(0,
                        8) + "->" + global.GetHexFromArr(Block.SumHash).substr(0, 8) + Engine.GetStrFromJumpArr(ArrJump) + "  " + StrSet, 3);
            }

            global.JINN_STAT.FindHeadCount += Count;
            global.JINN_STAT.MAXFindHeadCount = Math.max(global.JINN_STAT.MAXFindHeadCount, Count);
            if (Block && !Block.Hash)
        ...
        Engine.GetFirstEmptyBodyBlock = function (FirstBlockHeadNum, BlockSeed) {
            ...
            global.JINN_STAT.FindEmptyCount += Count;
            global.JINN_STAT.MAXFindEmptyCount = Math.max(global.JINN_STAT.MAXFindEmptyCount, Count);
            if (global.TEST_BLOCK_LIST) {
                var Block0 = Engine.GetFirstEmptyBodyBlock0(FirstBlockHeadNum, BlockSeed);
                if (Block && Block0 && Block0 !== Block && !global.IsEqArr(Block0.SumHash, Block.SumHash)) {
        ...
        Engine.IsExistBlockMain = function (Block) {
            ...
            if (!BlockDB)
                return 0;
            if (!global.IsEqArr(Block.TreeHash, BlockDB.TreeHash))
                return 0;
            if (!global.IsEqArr(Block.MinerHash, BlockDB.MinerHash))
                return 0;
            if (!global.IsEqArr(Block.PrevSumHash, BlockDB.PrevSumHash))
                return 0;
