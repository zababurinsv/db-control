...
//Engine context
    ...
    function InitClass(Engine) {
        debug(-5, '~~~~~~~~☯[(InitClass*)result]', Engine.ip)
        Engine.RndHash = global.GetRandomBytes(32);
        Engine.RndHashStr = global.GetHexFromArr(Engine.RndHash);
        Engine.ArrNodeName = [];
        Engine.StartHandShake = function (Child) {
            ...
            var Data = {
                Network: global.JINN_CONST.NETWORK_NAME,
                DepricatedShard: global.JINN_CONST.SHARD_NAME,
                ip: Engine.ip,
                port: Engine.port,
                DirectIP: Engine.DirectIP,
                RndHash: Engine.RndHash,
                RunVersionNum: global.UPDATE_CODE_VERSION_NUM,
                CodeVersionNum: global.CODE_VERSION.VersionNum,
                FindSelfIP: Child.FindSelfIP,
                RemoteIP: Child.ip,
                NameArr: Engine.ArrNodeName,
                Shard: global.JINN_CONST.SHARD_NAME,
                CurTime: Engine.GetTransferTime(Child),
            };
            Engine.Send("HANDSHAKE", Child, Data, Engine.OnHandShakeReturn);
        };
        Engine.HANDSHAKE_SEND = {
            Network: "str20",
            DepricatedShard: "str5",
            RemoteIP: "str30",
            port: "uint16",
            DirectIP: "byte",
            RndHash: "hash",
            ...
            FindSelfIP: "byte",
            NameArr: "arr40",
            Shard: global.SHARD_STR_TYPE,
            CurTime: "uint32",
        };
        Engine.HANDSHAKE_RET = {
            result: "byte",
            RndHash: "hash",
            RemoteIP: "str30",
            RunVersionNum: "uint",
        ...
        Engine.HANDSHAKE = function (Child, Data) {
            ...
            var AddrChild = {
                ...
                ShardName: Data.Shard
            };

            var StrError;
            if (Data.Network !== global.JINN_CONST.NETWORK_NAME) {
                StrError = "ERROR NETWORK_NAME=" + Data.Network;
                Engine.OnDeleteConnect(Child, StrError);
            } else if (global.IsEqArr(Engine.RndHash, Data.RndHash)) {
                if (Data.RemoteIP && Engine.ip === "0.0.0.0" && !global.IsLocalIP(Data.RemoteIP)) {
            ...
            } else if (!Engine.CanConnect(Child)) {
                StrError = "ERROR: NOT CAN CONNECT";
                Engine.OnDeleteConnect(Child, StrError);
            } else if (Engine.FindConnectByHash(Data.RndHash))
        ...
        Engine.DoMyAddres = function (Child, myip) {
            myip = global.GetNormalIP(myip);

            if (myip && Engine.ip === "0.0.0.0" && !global.IsLocalIP(myip)) {
