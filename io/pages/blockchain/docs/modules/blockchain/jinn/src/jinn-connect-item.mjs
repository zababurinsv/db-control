...
debug('[(self)a]', global.PROCESS_NAME)
    ...
    function InitClass(Engine) {
        debug(-3, '~~~~~~~~~~~~⛔[(InitClass*) Engine]', Engine.ip)
        Engine.RetNewConnectByIPPort = function (ip, port0) {
            var port = port0 >>> 0;
            debug(-3, '~~~~~~~⛔[(RetNewConnectByIPPort*) port]', port)
            if (!port) {
            ...
            var IDArr = global.CalcIDArr(ip, port);
            return Engine.NewConnect(IDArr, ip, port);
        };

        Engine.RetNewConnectByAddr = function (AddrItem) {
            debug(-3, '~~~~⛔[(RetNewConnectByAddr*) ip]', AddrItem.ip)
            let Item = Engine.NewConnect(AddrItem.IDArr, AddrItem.ip, AddrItem.port);
            if (Item) {
        ...
        Engine.NewConnect = function (IDArr, ip, port) {
            ...
            if (global.CompareArr(IDArr, Engine.IDStr) === 0) {
                return undefined;
            }

            var NodesCount = Engine.GetActualNodesCount();
            if (global.JINN_CONST.MAX_CONNECT_COUNT && NodesCount >= global.JINN_CONST.MAX_CONNECT_COUNT && !Engine.ROOT_NODE)
                return undefined;

            var IDStr = global.GetHexFromArr(IDArr);
        ...
        Engine.SetIPPort = function (Child, ip, port) {
            ...
            Child.IDArr = global.CalcIDArr(ip, port);
            Child.IDStr = global.GetHexFromArr(Child.IDArr);

            Child.ip = ip;
            Child.port = port;
            Engine.RecalcChildLevel(Child, 3);

            if (global.JINN_EXTERN.NodeRoot && ip === global.JINN_EXTERN.NodeRoot.ip && port === global.JINN_EXTERN.NodeRoot.port)
                Child.ROOT_NODE = 1;
        ...
        Engine.InitChild = function (Child) {
            ...
            Object.defineProperty(Child, "portweb", {
                get: function () {
                    if (this.AddrItem)
            ...
            Object.defineProperty(Child, "Score", {
                get: function () {
                    if (this.AddrItem)
            ...
            Object.defineProperty(Child, "TestExchangeTime", {
                get: function () {
                    if (this.AddrItem)
            ...
            Object.defineProperty(Child, "Debug", {
                set: function (value) {
                    if (this.AddrItem)
            ...
            Child.IsOpen = function () {
                debug(-3, '~~~~~~~~~~~⛔[(IsOpen*) ip]', Child.ip)
                let status = Engine.GetSocketStatus(Child)
                return ( status === 100);
            };
            Child.IsHot = function () {
                var LevelArr = Engine.GetLevelArr(Child);
                var ChildWas = LevelArr[this.Level];
                debug(-3, '~~~~~~~~~~~⛔[(IsHot*) ip]', Child.ip)
            ...
            Child.ToLog = function (Str, StartLevel) {
                var ID = global.GetNodeWarningID(Child);
        ...
        Engine.FindConnectedChildByArr = function (IDArr) {
            ...
            for (var i = 0; i < Engine.ConnectArray.length; i++) {
                ...
                if (global.IsEqArr(Child.IDArr, IDArr))
                    return Child;
            }
