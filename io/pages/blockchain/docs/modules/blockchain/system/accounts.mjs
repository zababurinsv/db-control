...
import accountsMerkleDbrow from './accounts-merkle-dbrow.mjs'
import accountsHash from "./accounts-hash.mjs"
...
//Accounts  and states engine
    ...
    const MerkleDBRow = accountsMerkleDbrow(global);
    ...
    global.FORMAT_MONEY_TRANSFER_BODY1 = global.FORMAT_MONEY_TRANSFER1.replace("Sign:arr64,", "");
...
//new
    ...
    global.FORMAT_MONEY_TRANSFER_BODY5 = global.FORMAT_MONEY_TRANSFER5.replace("Sign:arr64,", "");
    ...
    let AccountsHash = accountsHash(global)
    class AccountApp extends AccountsHash
    {
        constructor()
        {
            ...
            global.REGISTER_TR_DB(this.DBState, 10);
        ...
        Start(bClean)
        {
            ...
            global.ToLog("ACCOUNTS MAX_NUM:" + this.DBState.GetMaxNum())
        }
        ...
        ClearDataBase()
        {
            ...
            this.ClearHistory()
            this.ClearKeyValue()
            if(global.SHARD_PARAMS.GenesisAccountCreate)
                global.SHARD_PARAMS.GenesisAccountCreate()
            else
        ...
        {
            this.DBStateWriteInner({Num:0, PubKey:[], Value:{BlockNum:1, SumCOIN:0.95 * global.TOTAL_SUPPLY_TERA}, Name:"System account"}, 1)
            for(var i = 1; i < 8; i++)
                this.DBStateWriteInner({Num:i, PubKey:[], Value:{BlockNum:1}, Name:""})

            this.DBStateWriteInner({Num:8, PubKey: global.GetArrFromHex(ARR_PUB_KEY[0]), Value:{BlockNum:1, SumCOIN:0.05 * global.TOTAL_SUPPLY_TERA},
                Name:"Founder account"})
            this.DBStateWriteInner({Num:9, PubKey: global.GetArrFromHex(ARR_PUB_KEY[1]), Value:{BlockNum:1, SumCOIN:0}, Name:"Developer account"})
            for(var i = 10; i < BLOCK_PROCESSING_LENGTH2; i++)
                this.DBStateWriteInner({Num:i, PubKey: global.GetArrFromHex(ARR_PUB_KEY[i - 8]), Value:{BlockNum:1}, Name:""})
        }
        ...
        OnProcessBlockFinish(Block)
        {
            ...
            try
            {
                ...
                if(global.SHARD_PARAMS.DoCoinBaseTR)
                    global.SHARD_PARAMS.DoCoinBaseTR(Block)
                else
            ...
            {
                bErr = 1
                global.ToLogTx("BlockNum:" + Block.BlockNum + " - DoCoinBaseTR: " + e)
            }
        ...
        OnProcessTransaction(Block, Body, BlockNum, TrNum, ContextFrom)
        {
            ...
            switch(Type)
            {
                ...
                {
                    Result = this.TRTransferMoney(Block, Body, BlockNum, TrNum, global.FORMAT_MONEY_TRANSFER1, WorkStructTransfer);
                    break;
                }
                ...
                {
                    Result = this.TRTransferMoney5(Block, Body, BlockNum, TrNum, global.FORMAT_MONEY_TRANSFER5, WorkStructTransfer5);
                    break;
                }
        ...
        DoCoinBaseTR(Block)
        {
            ...
            if(Data && Data.Currency === 0 && Data.BlockNumCreate < Block.BlockNum)
            {
                ...
                if(Block.BlockNum >= NEW_FORMULA_START)
                {
                    ...
                    {
                        Sum = KTERA * SysBalance / global.TOTAL_SUPPLY_TERA;
                    }
                }
                else
                {
        ...
        GetPowTx(Body, BlockNum)
        {
            ...
            global.WriteUintToArrOnPos(FullHashTicket, BlockNum, 10)

            var HashPow = global.sha3(FullHashTicket, 32);
        ...
        GetFormatTransaction(Type)
        {
            ...
            switch(Type)
            {
                ...
                case TYPE_DEPRECATED_TRANSFER1:
                {
                    format = global.FORMAT_MONEY_TRANSFER1;
                    break;
                }
                case TYPE_DEPRECATED_TRANSFER2:
                {
                ...
                case TYPE_TRANSACTION_TRANSFER5:
                {
                    format = global.FORMAT_MONEY_TRANSFER5;
                    break;
                }
                case TYPE_TRANSACTION_ACC_HASH_OLD:
                case TYPE_TRANSACTION_ACC_HASH:
    ...
    global.REGISTER_SYS_DAPP(App, global.TYPE_TRANSACTION_CREATE);
    global.REGISTER_SYS_DAPP(App, global.TYPE_TRANSACTION_ACC_CHANGE);
    global.REGISTER_SYS_DAPP(App, TYPE_DEPRECATED_TRANSFER1);
    global.REGISTER_SYS_DAPP(App, TYPE_DEPRECATED_TRANSFER2);
    global.REGISTER_SYS_DAPP(App, global.TYPE_TRANSACTION_TRANSFER3);
    global.REGISTER_SYS_DAPP(App, global.TYPE_TRANSACTION_TRANSFER5);
    global.REGISTER_SYS_DAPP(App, global.TYPE_TRANSACTION_ACC_HASH);
    global.REGISTER_SYS_DAPP(App, global.TYPE_TRANSACTION_ACC_HASH_OLD);
}