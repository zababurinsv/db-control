...
'use strict';
import cMyTestBase from "./db/tera-db-tree.mjs";
...
debug('[(self)a]', global.PROCESS_NAME)
    ...
    function RunTestTreeDB(Count,CountM)
    {
        ...
        if(bReadOnly)
            return;
        let CMyTestBase = cMyTestBase(global)
        ...
        var TreeDB = new CMyTestBase("test-tree-datafile", {Shard:global.SHARD_STR_TYPE, Value:"uint", name:"str40", Delta:"uint", Msg:{Str:global.SHARD_STR_TYPE}},
        ...
        for(var m = 0; m < CountM; m++)
        {
            ...
            if(!bReadOnly)
            {
                ...
                {
                    var Name = "";
                    var NameL = 1 + global.random(20);
                    for(var n = 0; n < NameL; n++)
                        Name += "#";

                    var Shard = ShardStrArr[global.random(3)];
                    var Delta = global.random(1000000);
