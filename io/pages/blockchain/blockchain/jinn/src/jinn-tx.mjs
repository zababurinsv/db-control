...
//Engine context
    ...
    function InitClass(Engine) {
        ...
        Engine.AddCurrentProcessingTx = function (BlockNum, TxArr, bInnerAdd) {
            if (BlockNum < global.JINN_CONST.START_ADD_TX)
                return 0;
        ...
        Engine.SendTx = function (BlockNum) {
            ...
            for (var i = 0; i < ArrChilds.length; i++) {
                ...
                for (var t = 0; t < ArrTop.length; t++) {
                    ...
                    if (global.GetBit(Tx.TXSend, Level))
                        continue;
                    ...
                    if (DeltaTimeTT < global.JINN_CONST.MIN_TIME_SEND_TT_PERIOD) {
                        WasBreak = 1;
                        continue;
                    }

                    if (global.JINN_CONST.TEST_MODE_2 && !global.GetBit(Tx.TTTXReceive, Level))
                        continue;

                    if (global.GetBit(Tx.TTReceive, Level)) {
                        if (DeltaTimeTT < global.JINN_CONST.MAX_TIME_SEND_TT_PERIOD) {
                            WasBreak = 1;
                            continue;
                        }

                        if (global.GetBit(Tx.TTTXSend, Level))
                            continue;
                        TxArr.push({HashTicket: Tx.HashTicket, body: [], TxHolder: Tx});
                        global.JINN_STAT.TTTXSend++;
                        Tx.TTTXSend = global.SetBit(Tx.TTTXSend, Level);
                    } else {
                        TxArr.push({HashTicket: Tx.HashTicket, body: Tx.body});
                        global.JINN_STAT.TXSend++;
                        Tx.TXSend = global.SetBit(Tx.TXSend, Level);
                    }

                    ItemChild.TXSend++;
                    if (global.JINN_CONST.MAX_TRANSFER_TX && TxArr.length >= global.JINN_CONST.MAX_TRANSFER_TX) {
                        WasBreak = 1;
                        break;
                    }
                }
                ...
                function OnTxResult(Child, Data) {
                    if (Data && Data.ReqForLoad.length) {
                        ...
                        for (var i = 0; i < Data.ReqForLoad.length; i++) {
                            ...
                            if (Tx) {
                                var Tx2 = Tx.TxHolder;
                                Tx2.TXSend = global.ResetBit(Tx2.TXSend, Level);
                                Tx2.TTTXSend = global.ResetBit(Tx2.TTTXSend, Level);
                                Tx2.TTReceive = global.ResetBit(Tx2.TTReceive, Level);

                                global.JINN_STAT.TXReq++;
                            }
                        }
                        Engine.StepTaskTx[BlockNum] = 1;
                    }
        ...
        Engine.TRANSFERTX_SEND = {
            Reserve: "uint",
            BlockNum: "uint32",
            TxArr: [{HashTicket: "arr" + global.JINN_CONST.TT_TICKET_HASH_LENGTH, body: "tr"}]
        };
        ...
        Engine.TRANSFERTX = function (Child, Data) {
            ...
            if (!Engine.CanProcessBlock(BlockNum, global.JINN_CONST.STEP_TX)) {
            ...
            if (!Child || !Child.IsHot()) {
                global.JINN_STAT.ErrTx2++;
                return;
            }

            Engine.CheckSizeTransferTXArray(Child, TxArr, global.JINN_CONST.MAX_TRANSFER_TX);
            ...
            for (var t = 0; t < TxArr.length; t++) {
                ...
                if (ItemReceive.body.length === 0) {
                    ...
                    if (Find) {
                        Find.TTTXReceive = global.SetBit(Find.TTTXReceive, Level);
                    } else {
                        global.JINN_STAT.TxReceiveErr++;
                    }
                ...
                global.JINN_STAT.TxReceive++;
                global.JINN_STAT["TxReceive" + Level]++;
                ...
                if (!Engine.IsValidateTx(Tx, "TRANSFERTX", BlockNum))
                    continue;
                Tx.TXReceive = global.SetBit(Tx.TXReceive, Level);
        ...
        Engine.GetTxFromReceiveBody = function (Child, Tx, body, BlockNum, NumTx) {
            ...
            if (!Engine.IsValidateTx(TxRaw, "GetTxFromReceiveBody", BlockNum))
                return undefined;
            if (!global.IsEqArr(Tx.HashTicket, TxRaw.HashTicket)) {
