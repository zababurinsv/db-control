...
//Engine context
    function InitClass(Engine) {
        ...
        Engine.TimeMedian = new global.CTimeMedian(21, 200);

        Engine.TreeTimeStat = new global.RBTree(function (a, b) {
        ...
        Engine.CorrectTimeByDelta = function (NewDelta) {
            ...
            if (Engine.WasCorrectTime >= 1) {
                ...
                if (NewDeltaAbs >= global.JINN_CONST.CORRECT_TIME_TRIGGER) {
                    if (NewDeltaAbs > global.JINN_CONST.CORRECT_TIME_VALUE) {
                        if (NewDelta < 0)
                            NewDelta = -global.JINN_CONST.CORRECT_TIME_VALUE;
                        else
                            NewDelta = global.JINN_CONST.CORRECT_TIME_VALUE;
                    }
                } else {
            ...
            if (Math.abs(global.DELTA_CURRENT_TIME) > global.JINN_CONST.INFLATION_TIME_VALUE) {
                if (global.DELTA_CURRENT_TIME > 0)
                    NewDelta -= global.JINN_CONST.INFLATION_TIME_VALUE;
                else if (global.DELTA_CURRENT_TIME < 0)
                    NewDelta += global.JINN_CONST.INFLATION_TIME_VALUE;
            } else if (!NewDelta && Math.abs(global.DELTA_CURRENT_TIME) <= global.JINN_CONST.INFLATION_TIME_VALUE) {
            ...
            if (NewDelta && global.AUTO_CORRECT_TIME) {
                ...
                global.ToLog("SET TIME DELTA: " + Value + " ms (" + NewDelta + ")", 4);
        ...
        Engine.CorrectTimeByArr = function (Arr) {
            ...
            var MinCount = global.JINN_CONST.MIN_COUNT_FOR_CORRECT_TIME;
        ...
        Engine.DoTimeCorrect = function (bReglament) {
            ...
            if (Engine.TreeTimeStat.size < global.JINN_CONST.MIN_COUNT_FOR_CORRECT_TIME * 2)
                return;
        ...
        Engine.AddMaxHashToTimeStat = function (Item, BlockNum) {
            ...
            var BlockTimeCreate = global.CONSENSUS_PERIOD_TIME * global.JINN_CONST.STEP_NEW_BLOCK + global.JINN_CONST.DELTA_TIME_NEW_BLOCK;
            var BlockTimeNow = global.CONSENSUS_PERIOD_TIME + Item.StartTime - BlockNum * global.CONSENSUS_PERIOD_TIME - global.FIRST_TIME_BLOCK + global.DELTA_CURRENT_TIME;
            Item.Delta = (BlockTimeNow - BlockTimeCreate) / 1000;
            Engine.MaxTimeItem = Item;

            global.ToLog("BlockNum=" + BlockNum + " Power=" + Item.Power + " Time: " + BlockTimeNow + " - " + Delta + " = " + Item.Delta, 5);
        };
        ...
        Engine.GetTransferTime = function (Child) {
            ...
            if (global.CURRENT_OS_IS_LINUX)
                CurTime = global.GetCurrentTime() % CLUSTER_TIME_PERIOD;
            return CurTime;
        };
        ...
        Engine.SetTransferTime = function (Child, NodeTime) {
            ...
            var CurTime = global.GetCurrentTime() % CLUSTER_TIME_PERIOD;
            ...
            if (Math.abs(Delta) < CLUSTER_TIME_PERIOD / 2) {
                ...
                if (Math.abs(AvgDelta) > 300) {
                    global.ToLog("----- Node " + Child.Name + " set cluster time Delta=" + AvgDelta, 3);
