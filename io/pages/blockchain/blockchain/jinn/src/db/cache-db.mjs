...
debug('[(self)a]', global.PROCESS_NAME)
    ...
    class CCache {
        constructor(MaxCacheSize, FCompare)
        {
            ...
            this.CacheDBTree = new global.RBTree(FCompare)
            this.VersionDBTree = new global.RBTree(function (a,b)
            {
        ...
        SetMaxSizeCache(MaxCacheSize)
        {
            if(typeof MaxCacheSize !== "number")
                global.ToLogTrace("Error type of MaxCacheSize = " + MaxCacheSize)
            this.MaxCacheSize = MaxCacheSize
        }
        ...
        CheckDBBlockCacheSize(Size)
        {
            ...
            global.JINN_STAT.MAXCacheLength = Math.max(global.JINN_STAT.MAXCacheLength, WasSize)
        ...
        FindItemInCache(CacheIndex)
        {
            ...
            global.JINN_STAT.CacheErrDB++

            var CacheControlNum = 0;
            if(CacheControlNum)
            {
                if(!this.CacheErrorList || global.JINN_STAT.CacheErrDB === 1)
                    this.CacheErrorList = []
                this.CacheErrorList.push(CacheIndex)

                if(global.JINN_STAT.CacheErrDB === CacheControlNum)
                {
                    var Item = this.CacheDBTree.max();
                    global.ToLogTrace("CacheErrDB: MAX=" + Item.CacheIndex + " Arr=" + this.CacheErrorList)
                }
            }
