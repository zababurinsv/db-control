...
debug('💧[(self)a]', global.PROCESS_NAME)
    ...
    global.JINN_VERSION_DB = 3;
    global.NUM_FOR_MAX_BLOCK =  - 1;
    ...
    if(global.Init_DB_HEADER_FORMAT) {
        global.Init_DB_HEADER_FORMAT(global.DB_HEADER_FORMAT);
    }
    ...
    class CDBChain
    {
        constructor(EngineID, FCalcBlockHash)
        {
            ...
            this.DBMainIndex = new global.CDBRow("main-index", {MainPosition:"uint"}, !BWRITE_MODE, "BlockNum", 10, 0, EngineID)
            this.DBChainIndex = new global.CDBRow("chain-index", {LastPosition:"uint"}, !BWRITE_MODE, "BlockNum", 10, 0, EngineID)
            this.DBBlockHeader = new global.CDBItem("block-data", global.DB_HEADER_FORMAT, !BWRITE_MODE, EngineID, 1)
            this.DBBlockBody = new global.CDBItem("block-data", BODY_FORMAT, !BWRITE_MODE, EngineID)
        ...
        WriteBlock(Block)
        {
            ...
            if(global.TEST_DB_BLOCK && !Block.Position)
            {
                var Find2 = this.FindBlockByHash(Block.BlockNum, Block.SumHash);
                if(Find2)
                {
                    global.ToLogTrace("Find double block on Block=" + Block.BlockNum, 2)
                }
            }
            ...
            {
                if(Block.Position === 0)
                    global.ToLogTrace("Block.Position===0 on Block=" + Block.BlockNum)
                if(isNaN(Item.LastPosition))
                    global.ToLogTrace("Is NAN LastPosition on Block=" + Block.BlockNum)
            ...
            if(!global.IsZeroArr(Block.TreeHash) && Block.TxData && !Block.TxPosition)
            {
                ...
                if(!Block.TxPosition)
                {
                    global.ToLogTrace("Error write tx on Block=" + Block.BlockNum + "  TxPosition=" + Block.TxPosition)
                }
            }

            global.JINN_STAT.WriteBlock++
            Block.VersionDB = global.JINN_VERSION_DB
        ...
        {
            global.JINN_STAT.ReadBlock++

            var Block = this.DBBlockHeader.Read(Position);
            if(!Block) {
                return undefined;
            }
            // debug(-4,'💧[(ReadBlock*)Block.VersionDB, global.JINN_VERSION_DB]', Block.VersionDB, global.JINN_VERSION_DB)
            if(Block.VersionDB !== global.JINN_VERSION_DB)
            {
                global.ToLogTrace("Error version DB: " + Block.VersionDB + "/" + global.JINN_VERSION_DB)
                return undefined;
            }
            if(!global.IsZeroArr(Block.TestZero))
            {
                ToLogOne("Error TestZero = " + global.GetHexFromArr(Block.TestZero) + " on PosFile=" + Position)
            }
        ...
        WriteBlockMain(Block)
        {
            ...
            if(!Block.TxPosition && !Block.TxData && !global.IsZeroArr(Block.TreeHash))
            {

                global.ToLogTrace("Error - empty TxData on Block=" + Block.BlockNum)
                return 0;
            }
            if(Block.BlockNum > global.JINN_CONST.BLOCK_GENESIS_COUNT)
            {
                ...
                if(!PrevBlock)
                {
                    global.ToLogTrace("Error read PrevBlock on block=" + Block.BlockNum)
                    return 0;
                }
                if(!global.IsEqArr(PrevBlock.SumHash, Block.PrevSumHash))
                {
                    ToLog("Error PrevSumHash on block=" + Block.BlockNum + "  " + global.GetHexFromArr(Block.PrevSumHash) + "/" + global.GetHexFromArr(PrevBlock.SumHash),
                        3)
                    return 0;
                }
            }
            ...
            global.JINN_STAT.MaxSumPow = this.MaxSumPow
            ...
            {
                global.ToLogTrace("WriteBlockMain Zero TxPosition on Block=" + Block.BlockNum + "  TxPosition=" + Block.TxPosition + "  TxCount" + Block.TxCount)
                return 0;
            }
        ...
        TruncateMain(LastBlockNum)
        {
            ...
            this.WriteMainIndex(global.NUM_FOR_MAX_BLOCK, LastBlockNum)
        }
        GetMaxNumBlockDB()
        {
        ...
        GetMaxNumBlockDBInner()
        {
            ...
            var Item = this.DBMainIndex.Read(global.NUM_FOR_MAX_BLOCK);
            if(!Item)
            {
        ...
        GetBodyFilePosition(LastPosition, BlockNum, TreeHash, TxData)
        {
            ...
            while(Position)
            {
                ...
                if(Block.TxPosition && global.IsEqArr(TreeHash, Block.TreeHash))
                {
        ...
        {
            if(BlockNum < global.JINN_CONST.BLOCK_GENESIS_COUNT)
            {
                var Item = this.ReadMainIndex(BlockNum);
                if(Item)
                    return Item.MainPosition;
        ...
        GetChainArrByNum(BlockNum)
        {
            ...
            global.JINN_STAT.MAXChainHeight = Math.max(global.JINN_STAT.MAXChainHeight, Arr.length)
            return Arr;
        }

        FindBlockByHash(BlockNum, Hash)
        {
            if(BlockNum > 0 && global.IsZeroArr(Hash))
            {
            ...
            while(Position)
            {
                ...
                if(global.IsEqArr(Hash, Block.SumHash))
                {
                    return Block;
                }
                if(global.IsEqArr(Hash, Block.Hash))
                {
                    return Block;
                }
                if(global.IsEqArr(Hash, Block.TreeHash))
                {
                    return Block;
                }
            ...
            global.JINN_STAT.MAXChainHeight = Math.max(global.JINN_STAT.MAXChainHeight, Count)
        ...
        GetPrevBlockDB(Block, bRaw, bMustHave)
        {
            if(Block.PrevBlockPosition)
            {
                ...
                if(global.TEST_DB_BLOCK && PrevBlock && !global.IsEqArr(Block.PrevSumHash, PrevBlock.SumHash))
                    PrevBlock = undefined
            ...
            if(PrevBlock)
            {
                if(global.JINN_CONST.HOT_BLOCK_DELTA && Block.BlockNum > global.JINN_EXTERN.GetCurrentBlockNumByTime() - global.JINN_CONST.HOT_BLOCK_DELTA)
                    return PrevBlock;
        ...
        GetPrevBlockDBInner(Block)
        {
            ...
            if(global.IsZeroArr(Block.PrevSumHash))
            {
        ...
        {
            if(StrType === "H" && global.JINN_CONST.HOT_BLOCK_DELTA && BlockSeed.BlockNum > global.JINN_EXTERN.GetCurrentBlockNumByTime() - global.JINN_CONST.HOT_BLOCK_DELTA)
                return 0;
        ...
        SaveChainToDBInner(BlockHead, BlockSeed)
        {
            ...
            while(Block)
            {
                ...
                if(!PrevBlock)
                {
                    ToLog("Error PrevBlock on Block=" + Block.BlockNum + " BlockHead=" + BlockHead.BlockNum + " PrevSumHash=" + global.GetHexFromArr(Block.PrevSumHash),
                        4)
                    return  - 1;
                }
                Block = PrevBlock
        ...
        {
            if(StartNum < global.JINN_CONST.BLOCK_GENESIS_COUNT)
            {
                this.Clear()
                return;
            }
        ...
        {
            var StartHotBlockNum = BlockSeed.BlockNum - global.JINN_CONST.HOT_BLOCK_DELTA;
