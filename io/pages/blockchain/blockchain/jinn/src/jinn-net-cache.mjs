...
//Engine context
    ...
    function DoNode(Engine) {
        ...
        var BlockNum = LastBlockNum - global.JINN_CONST.STEP_CLEAR_MEM;
    ...
    function InitClass(Engine) {
        ...
        Engine.ProcessBlockOnSend = function (Child, Block, TxData) {
            ...
            var ArrFull = [];
            var ArrTtTx = [];
            if (DeltaBlockNum >= global.JINN_CONST.STEP_CLEAR_MEM - global.JINN_CONST.MAX_DELTA_PROCESSING || Child.StartHotTransferNum + 5 >= BlockNum) {
                ...
                for (var i = 0; i < TxData.length; i++) {
                    ...
                    if (GetBit(Tx.TXReceive, Level) || GetBit(Tx.TXSend, Level) || GetBit(Tx.TTTXReceive, Level)) {
                        ...
                        Size += global.JINN_CONST.TT_TICKET_HASH_LENGTH + 2;
                    } else {
                        ...
                        ArrTtTx.push(Tx);
                        Size += global.JINN_CONST.TT_TICKET_HASH_LENGTH + Tx.body.length + 2;
        ...
        Engine.ProcessBlockOnReceive = function (Child, Block) {
            ...
            if (Block.ArrFull.length) {
                ...
                for (var i = 0; i < ArrTtTx.length; i++) {
                    ...
                    var Item = ArrTtTx[i];
                    if (Item.body.length)
                        global.JINN_STAT.TxCacheErr++;
                    else
                        global.JINN_STAT.TxCache2++;
        ...
        Engine.SetReceiveBits = function (Child, Block) {
            ...
            var BlockNum = Block.BlockNum;
            var DeltaBlockNum = Engine.CurrentBlockNum - BlockNum;
            if (DeltaBlockNum >= global.JINN_CONST.STEP_CLEAR_MEM)
                return;
        ...
        Engine.PackMaxHashOnSend = function (Child, BlockNum, Arr, ArrRepeat) {
            ...
            for (var a = 0; a < Arr.length; a++) {
                ...
                if (Status.LoadH && global.IsEqArr(Status.LoadH, Status.Hash)) {
                    Status.Mode |= 128;
                    Status.LoadH = global.ZERO_ARR_32;
                }
            }
        };
        ...
        Engine.InitChildNext = function (Child) {
            Child.SendBodyTimeCache = new global.CTimeCache(function (a, b) {
