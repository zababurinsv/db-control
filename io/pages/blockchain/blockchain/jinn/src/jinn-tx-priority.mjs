...
//Engine context
    function InitClass(Engine) {
        Engine.InitPriorityTree = function () {
            Engine.SenderActTree = new global.RBTree(function (a, b) {
                return a.BlockNum - b.BlockNum;
            });
            Engine.SenderRestTree = new global.RBTree(function (a, b) {
        ...
        Engine.PrepareLastStatFromDB = function (HeadBlockNum) {
            ...
            if (!HeadBlockNum)
                HeadBlockNum = Engine.CurrentBlockNum - global.JINN_CONST.STEP_SAVE;
            if (HeadBlockNum < global.JINN_CONST.BLOCK_GENESIS_COUNT)
                return 0;

            var Num = HeadBlockNum;
            var Count = 0;
            while (Count < global.JINN_CONST.TX_PRIORITY_LENGTH) {
                ...
                if (Find) {
                    if (global.IsEqArr(Find.Hash, BlockDB.Hash))
                        break;

                    if (global.IsEqArr(Find.TreeHash, BlockDB.TreeHash)) {
            ...
            var MinNum = HeadBlockNum - global.JINN_CONST.TX_PRIORITY_LENGTH;
            var Data;
            while (1) {
        ...
        Engine.SortBlockPriority = function (Block) {
            var NumNew = Engine.CurrentBlockNum - global.JINN_CONST.STEP_NEW_BLOCK;
            if (!global.JINN_CONST.TX_PRIORITY_MODE || !Block || !Block.TxData || Block.TxData.length <= 0)
                return Engine.SortBlock(Block);

            var NumSave = Engine.CurrentBlockNum - global.JINN_CONST.STEP_SAVE;
        ...
        Engine.SortArrPriority = function (Arr, BlockNum, bBlockSort) {
            ...
            Arr.sort(function (a, b) {
                if (typeof a.SenderNum !== "number")
                    global.ToLogTrace("Error type a.SenderNum");
                if (typeof b.SenderNum !== "number")
                    global.ToLogTrace("Error type b.SenderNum");
            ...
            for (var i = 0; i < Arr.length; i++) {
                ...
                if (typeof Tx.CurCountTX !== "number")
                    global.ToLogTrace("Error type Tx.CurCountTX");
            ...
            Arr.sort(function (a, b) {
                ...
                if (typeof a.Priority !== "number")
                    global.ToLogTrace("Error type a.Priority");
                if (typeof b.Priority !== "number")
                    global.ToLogTrace("Error type b.Priority");
        ...
        Engine.LogArrPriority = function (Name, Arr, BlockNum) {
            ...
            for (var i = 0; i < Arr.length; i++) {
                var Tx = Arr[i];
                Arr2.push({
                    SenderNum: Tx.SenderNum,
                    Priority: Tx.Priority,
                    OperationID: Tx.OperationID,
                    BaseValue: Tx.BaseValue,
                    CanBaseTx: Tx.CanBaseTx,
                    CountTX: Tx.CountTX,
                    CurCountTX: Tx.CurCountTX,
                });
            }
