...
debug('[(self)a]', global.PROCESS_NAME)
    ...
    global.WebApi2.GenerateKeys = function (Params) {
        console.log('~~~~~~~ API V2 ~~~~~~~', Params)
    ...
    global.WebApi2.CreateAccount = function (Params,response) {
        if(typeof Params === "object" && Params.Name && Params.PrivKey) {
            var KeyPair = crypto.createECDH('secp256k1');
            KeyPair.setPrivateKey(Buffer.from(GetArrFromHex(Params.PrivKey)));
            var PubKey = KeyPair.getPublicKey('', 'compressed');
            var TR = {Type:global.TYPE_TRANSACTION_CREATE, Currency:Params.Currency, PubKey:PubKey, Name:Params.Name, Smart:Params.Smart, };
    ...
    global.WebApi2.Send = function (Params,response,A,request,nJsonRet) {
        ...
        var BVersion= global.GETVERSION(CurBlockNum);
        if(BVersion<2) {
            Format= global.FORMAT_MONEY_TRANSFER3;
            TR = {
                Type: global.TYPE_TRANSACTION_TRANSFER3, Version: 4, OperationID: OperationID, FromID: FromNum, DeprecatedOperationID: global.START_CODE_VERSION_NUM, To: [{
            ...
            TR = {
                Type: global.TYPE_TRANSACTION_TRANSFER5,
                Version: 4,
        ...
        TR.Sign = ACCOUNTS.GetSignTransferTx(TR, global.GetArrFromHex(Params.FromPrivKey));


        var Body = BufLib.GetBufferFromObject(TR, Format, global.GetTxSize(TR), {}, 1);
        Body = Body.slice(0, Body.len + 12);
...
//     {
//         global.CopyObjKeys(MParams,Params.TokenID);
...
// }
    ...
    global.WebApi2.SendCall = function (Params,response,A,request) {
        if(typeof Params !== "object")
            return {result:0};
    ...
    global.WebApi2.GetBalance = function (Params,response) {
        if(typeof Params === "object")
        {
            ...
            if(arr.length)
            {
                ...
                else
                {
                    Result = {
                        ...
                        PubKey: global.GetHexFromArr(Account.PubKey),
                        Meta: Params.Meta
                    };
                }
    ...
    global.WebApi2.GetTransaction = function (Params) {
    ...
    global.WebApi2.GetHistoryTransactions = function (Params) {
    ...
    global.WebApi2.CreateRawTransaction = function (Params) {
        if(typeof Params === "object")
        {
    ...
    global.WebApi2.SignRawTransaction = function (Params) {
    ...
    global.WebApi2.SendRawTransaction = function (Params,response) {
        if(typeof Params === "object" && Params.Tx)
        {
            if(typeof Params.Tx !== "object")
                return {result:0, Meta:Params.Meta, text:"Params.Tx required"};
            ...
            var TR = Params.Tx;
            TxHexToArr(TR);
            var Body = BufLib.GetBufferFromObject(TR, global.FORMAT_MONEY_TRANSFER3, GetTxSize(TR), {}, 1);
            Body = Body.slice(0, Body.len + 12);
    ...
    function GetBlockNumTr(arr) {
        var BlockNum = global.GetCurrentBlockNumByTime(0);
        return BlockNum;
    }

    function PrepareTxID(TR,arr) {
        var BlockNum = GetBlockNumTr(arr);
        var TxID = global.CreateTxID(arr.slice(0, arr.length - 12), BlockNum);
        TR._TxID = global.GetHexFromArr(TxID);
        TR._BlockNum = BlockNum;
        return 0;
    }

    function SendTransaction(Body,TR,Confirm,F) {
        var Params = {HexValue:global.GetHexFromArr(Body),Confirm:Confirm,F:1};
        if(process.RunRPC)
        {
